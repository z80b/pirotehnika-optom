...
class OrderStateCore extends ObjectModel
{
    ...
    /** @var bool Можно ли заказы с этим статусом отправлять В заказы поставщику покупателя */
    public $for_move ;
    ...
    public static $definition = array(
        ...
        'multilang' => true,
        'fields' => array(
            'order_state_groups' =>    array('type' => self::TYPE_STRING, 'validate' => 'isString'),
            'id_shop' =>    array('type' => self::TYPE_INT, 'validate' => 'isUnsignedId'),
            'for_all_shop' => array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),
            'sort' =>    array('type' => self::TYPE_INT, 'validate' => 'isUnsignedId'),
            ...
            'is_to_show' =>    array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),
            'for_move' =>    array('type' => self::TYPE_BOOL, 'validate' => 'isBool'),

            /* Lang fields */
            'name' =>        array('type' => self::TYPE_STRING, 'lang' => true, 'validate' => 'isGenericName', 'required' => true, 'size' => 64),
            'name_front' =>        array('type' => self::TYPE_STRING, 'lang' => true, 'validate' => 'isGenericName', 'required' => true, 'size' => 64),
    ...
    */
    public static function getOrderStates($id_lang, $id_shop = 0, $id_state = 0)
    {
		if ($id_state > 0) {
				...
				WHERE `deleted` = 0 and `is_to_show` = 1 AND ((`id_shop` = 0) OR (`for_all_shop` = 1) OR (`id_shop` = '.$id_shop.') OR (os.`id_order_state` = '.$id_state.'))
				ORDER BY `sort` ASC');
				$result_needed = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
				SELECT *
				FROM `'._DB_PREFIX_.'order_state_next`');
				
				$arr = [];
				// собираем id_needed
				foreach($result_needed as $value) {
					if($id_state == $value['id_order_state_current'] && $id_shop == $value['id_shop']) {
							$arr[] = $value['id_order_state_next'];
					}
				}
				if(!empty($arr)) { // фильтруем при необходимости
					if (!in_array($id_state, $arr)) {
						$arr[] = $id_state;
					}
					$results = array_filter($results, function($item) use ($arr) {return in_array($item['id_order_state'], $arr);});
				}
				return $results;
			
		} else {
			$cache_id = 'OrderState::getOrderStates_'.(int)$id_lang.'_'.(int)$id_shop;
			if (!Cache::isStored($cache_id)) {
				$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
				SELECT *
				FROM `'._DB_PREFIX_.'order_state` os
				LEFT JOIN `'._DB_PREFIX_.'order_state_lang` osl ON (os.`id_order_state` = osl.`id_order_state` AND osl.`id_lang` = '.(int)$id_lang.')
				WHERE `deleted` = 0 and `is_to_show` = 1 AND ((`id_shop` = 0) OR (`for_all_shop` = 1) OR (`id_shop` = '.$id_shop.'))
				ORDER BY `name` ASC');
