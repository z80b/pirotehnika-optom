...
class OrderDetailCore extends ObjectModel
{
    ...
    /** @var float */
    public $my_price_discount;

    /** @var float */
    public $my_real_discount;
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'my_price_discount' =>    array('type' => self::TYPE_FLOAT, 'validate' => 'isFloat'),
            'my_real_discount' =>    array('type' => self::TYPE_FLOAT, 'validate' => 'isFloat'),
...
//        $this->original_product_price = Product::getPriceStatic($product['id_product'], false, (int)$product['id_product_attribute'], 6, null, false, false, 1, false, null, null, null, $null, true, true, $this->context);
        $this->original_product_price = Product::getPriceRetail($product['id_product']);
		...
		$this->my_real_discount = 100 - $this->unit_price_tax_incl / $this->original_product_price *100;
				if ($product['country_prois'] == 1) {
					...
					$this->my_price_discount = MIN($this->context->customer->skid_2, $product['max_discount']);
				} elseif ($product['country_prois'] == 3) {
					$this->my_price_discount = MIN($this->context->customer->skid_3, $product['max_discount']);
				} else {
