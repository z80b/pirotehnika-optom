...
<div id="template_area">
    <div id="template_header_part">
        ...
        <div class="revolution-template-switcher">
            <span data-showgroup="revolution-all-slides-templates" class="revolution-templatebutton selected"><?php _e('All Slides', 'revslider'); ?></span>
    ...
    <div class="revolution-premium-templates revolution-template-groups" style="display: none">
        ...
        if (!empty($tp_template_slider)) {
            foreach ($tp_template_slider as $m_slider) {
                ...
                if (!@RevsliderPrestashop::getIsset($m_slider['installed']) && !@RevsliderPrestashop::getIsset($m_slider['is_new'])) {
                    ...
                    if (!empty($c_slides)) {
                        ...
                        if (@RevsliderPrestashop::getIsset($m_slider['filter'])) {
                        ...
                        ?>">							
                                 ...
                                 if (@RevsliderPrestashop::getIsset($m_slider['preview']) && $m_slider['preview'] !== '') {
    ...
    <div class="revolution-all-slides-templates revolution-template-groups" style="display:block;">
        <?php
        if (!empty($all_slider)) {
            foreach ($all_slider as $c_slider) {
                ...
                if (!empty($c_slides)) {
                    ...
                    ?>
                    <div class="template_group_wrappers <?php //if(@RevsliderPrestashop::getIsset($c_slider['filter'])){ echo implode(' ', $c_slider['filter']); } ?>">
                        <?php
...
if (!@RevsliderPrestashop::getIsset($rs_disable_template_script)) {
    ...
    <div id="dialog_import_template_slide" title="<?php _e("Import Template Slide", 'revslider');
        ...
        <form action="<?php echo RevSliderBase::$url_ajax;
            ...
            <span style="margin-top:45px;display:block"><input type="submit" class="rs-import-slider-button button-primary revblue tp-be-button" value="<?php _e("Import Template Slide", 'revslider');

              ?>"></span>
            <span class="tp-clearfix"></span>
            <span style="font-weight: 700;"><?php _e("Note: style templates will be updated if they exist!", 'revslider');

              ?></span><br><br>
            <table style="display: none;">
                <tr>
                    <td><?php _e("Custom Animations:", 'revslider');

                    ?></td>
                    <td><input type="radio" name="update_animations" value="true" checked="checked"> <?php _e("overwrite", 'revslider');
                ...
                <tr>
                    <td><?php _e("Static Styles:", 'revslider');

                    ?></td>
                    <td><input type="radio" name="update_static_captions" value="true"> <?php _e("overwrite", 'revslider');

                    ?></td>
                    <td><input type="radio" name="update_static_captions" value="false"> <?php _e("append", 'revslider');

                    ?></td>
                    <td><input type="radio" name="update_static_captions" value="none" checked="checked"> <?php _e("ignore", 'revslider');
