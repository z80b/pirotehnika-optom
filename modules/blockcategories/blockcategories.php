...
class BlockCategories extends Module
{
	...
	public function getTree($resultParents, $resultIds, $maxDepth, $id_category = null, $currentDepth = 0)
	{
		...
		$return = array(
			...
			'name' => $name,
			'desc'=> $desc,
			'sum'=> 0,//(null !== Cart::getCategoriesSum($id_category)) ? Cart::getCategoriesSum($id_category) : 0,
			'children' => $children
		);
		return $return;
	}
	...
	public function hookLeftColumn($params)
	{
		...
		if (!$this->isCached('blockcategories.tpl', $cacheId))
		{
            ...
            $cats = $blockCategTree['children'];
            $cur = $category;
            function opensId($cats, $cur){
                $res = array();
                foreach($cats as $cat){
                    if($cur->id == $cat['id']){
                        $res[] = $cat['id'];
                    ...
                    $children = array();
                    if(isset($cat['children'])) $children = $cat['children'];
                    if(!empty($children)){
                        $tmp = opensId($children, $cur);
                        if(!empty($tmp)){
                            $res = $tmp;
                            $res[] = $cat['id'];
                        }
            ...
            $opens = opensId($cats, $cur);
            $this->smarty->assign('openIds', $opens);
