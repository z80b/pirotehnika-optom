...
class ManufacturerCore extends ObjectModel
{
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'short_name' =>          array('type' => self::TYPE_STRING, 'validate' => 'isString', 'size' => 20),
    ...
    public static function getMyManufacturers($id_shop = 0, $active = true)
    {
        if (!$id_shop) {
			if (!$context) {
		...
		right JOIN `'._DB_PREFIX_.'manufacturer_shop` ms ON (m.`id_manufacturer` = ms.`id_manufacturer` AND ms.`id_shop` = '.(int)$id_shop.')
		'.($active ? ' WHERE m.`active` = 1' : '').' ORDER BY m.`name` ASC');
        if ($manufacturers === false) {
            return false;
        }
