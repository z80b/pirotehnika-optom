        ...
        $this->fields_options = array(
            'general' => array(
                ...
                'fields' =>    array(
                    ...
                    'PS_HARD_REGISTRATION' => array(
                        'title' => $this->l('Запретить новым клиентам покупки без проверки его данных'),
                        'desc' => $this->l('Если выбрано Да, то новый клиент сможет делать покупки только после проверки его данных и включения разрешения на это в карточке клиента.'),
                        'validation' => 'isBool',
                        'cast' => 'intval',
                        'type' => 'bool'
                    ),
                    'PS_EASY_CODE_REGISTRATION' => array(
                        'title' => $this->l('Разрешить новым клиентам покупки без проверки его данных, если им введен Промокод при регистрации'),
                        'desc' => $this->l('Если выбрано Да, то новый клиент сможет делать покупки сразу после регистрации, если им будет введен Промокод, указанный ниже.'),
                        'validation' => 'isBool',
                        'cast' => 'intval',
                        'type' => 'bool'
                    ),
					'PS_EASY_CODE_REGISTRATION_PROMO' => array(
						'title' => $this->l('Промокод для регистрации'),
						'desc' => $this->l('Укажите Промокод для регистрации (смотри настройку выше)'),
						'type' => 'text',
						'class' => 'fixed-width-xxl'
					),
                    'PS_STORE_OWN_FOLDER' => array(
                        'title' => $this->l('Хранить название персональной папки клиента'),
                        'desc' => $this->l('Если выбрано Да, то в карточке клиента будет отображаться и храниться путь к персональной папке этого клиента.'),
                        'validation' => 'isBool',
                        'cast' => 'intval',
                        'type' => 'bool'
                    ),
                    ...
                    'PS_OWN_NAME' => array(
                        'title' => $this->l('Использовать собственное обозначение покупателя'),
                        'hint' => $this->l('Для постоянных клиентов можно задать собственное обозначение вместо ФИО.'),
                        'validation' => 'isBool',
                        'cast' => 'intval',
                        'type' => 'bool'
                    ),
					'PS_OWN_NAME_DEFAULT' => array(
						'title' => $this->l('Собственное обозначение для новых покупателей'),
						'desc' => $this->l('Это обозначение будет присвоено новому покупателю, который зарегистрировался сам.'),
						'type' => 'text',
						'class' => 'fixed-width-xxl'
					),
                    'PS_OWN_COUNTRY_DISCOUNT' => array(
                        'title' => $this->l('Привязывать к покупателю индивидуальные скидки на товары'),
                        'hint' => $this->l('Для каждого клиента можно указать индивидуальные скидки на товары разных стран происхождения.'),
                        'validation' => 'isBool',
                        'cast' => 'intval',
                        'type' => 'bool'
                    ),
                    'PS_IS_SHOW_DISCOUNT' => array(
                        'title' => $this->l('Разрешить покупателям видеть цену товаров со скидками'),
                        'hint' => $this->l('Если включено, то покупатель сможет включать / отключать показ цен со скидкой.'),
                        'validation' => 'isBool',
                        'cast' => 'intval',
                        'type' => 'bool'
                    ),
