...
class CarrierCore extends ObjectModel
{
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'my_type' =>        array('type' => self::TYPE_INT, 'validate' => 'isUnsignedInt'),
    ...
    */
    ...
    public static function getDeliveredCountries($id_lang, $active_countries = false, $active_carriers = false, $contain_states = null)
    {
		...
		RIGHT JOIN `'._DB_PREFIX_.'zone_shop` zzs ON (zzs.id_zone = s.id_zone AND zzs.`id_shop` = '.(int)Shop::getContextShopID().')
		ORDER BY s.`name` ASC');
    ...
    public static function getMyCarriers($id_shop = 0)
    {
        return Db::getInstance()->executeS('
			SELECT t.`id_carrier`, `name`
			FROM `'._DB_PREFIX_.'carrier` t
			LEFT JOIN `'._DB_PREFIX_.'carrier_shop` ts ON (ts.`id_carrier` = t.`id_carrier` )
			WHERE ts.`id_shop` = '.$id_shop.' AND t.`deleted` = 0 AND t.`my_type` > 0
