		...
		<table{if $table_id} id="table-{$table_id}"{/if} class="table{if $table_dnd} tableDnD{/if} {$table}" >
			...
			{if !$simple_header && $show_filters}
				<tr class="nodrag nodrop filter {if $row_hover}row_hover{/if}">
					...
					{foreach $fields_display AS $key => $params}
						<th  class="{if isset($params.align)}{$params.align}{/if} {if isset($params.class)} {$params.class}{/if}" >
							...
							{else}
								...
								{elseif $params.type == 'select'}
									...
									{/if}
								{else}
									<input type="text" class="filter {if isset($params.class)} {$params.class}{/if}" name="{$list_id}Filter_{if isset($params.filter_key)}{$params.filter_key}{else}{$key}{/if}" value="{$params.value|escape:'html':'UTF-8'}" {if isset($params.width) && $params.width != 'auto'} style="width:{$params.width}px"{/if} />
								{/if}
							{/if}
						</th>
					{/foreach}
