	...
	function refreshMyList()
	{
					...
					$.each(my_orders, function() {
						if ((tek_customer == 0) || ((tek_customer > 0) && ((tek_customer == this.id_customer) || (tek_order == this.order_number))))  {
							...
							if ((show_filter == 0) || ((show_filter == 1) && (Math.abs(delta) < 1)) || ((show_filter == 2) && (	paym_zak == this.order_number))) {
								html_orders += '<tr>';
			...
			<div id="carts">
				...
				<div id="old_carts_orders" class="tab-content panel collapse in">
					<div id="nonOrderedCarts" class="tab-pane active">
						...
						<br>
						<div class="pull-left">
								<button type="button" class="btn btn-primary" id="showsum" />
								<i class="icon-ok text-success"></i>
								{l s='Показать с похожими суммами'}
								<button type="button" class="btn btn-primary" id="showzak" />
								<i class="icon-ok text-success"></i>
								{l s='Показать только указанный заказ'}
								<button type="button" class="btn btn-primary" id="showall" />
								<i class="icon-ok text-success"></i>
								{l s='Показать все'}
				...
				<input type="hidden" id="summa_no_old" name="summa_no" value="{$currentSumNo}"  disabled="disabled"/>
						...
						<div class="pull-right">
								<button type="button" class="btn btn-primary" id="rasp1" />
								<i class="icon-ok text-success"></i>
								{l s='Распределить по порядку'}
								<button type="button" class="btn btn-primary" id="raspotm" />
								<i class="icon-ok text-success"></i>
								{l s='Распределить по отмеченным'}
								<button type="button" class="btn btn-primary" id="raspclear" />
								<i class="icon-ok text-success"></i>
								{l s='Очистить'}
								<button type="button" class="btn btn-primary" id="raspprim" />
								<i class="icon-ok text-success"></i>
								{l s='Применить'}
						</div>
						<table class="table">
