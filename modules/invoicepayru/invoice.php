...
class SPDF extends MYPDF
{
	public static function invoice($id_order, $mode = 'I')
	{
		...
		$pdf->SetTitle('Счет на оплату № '.$order->order_number);
		...
		$pdf->SetMargins(15, 0, 15); // отступы (слева, сверху, справа)
		...
		$org_name = Configuration::get('INVOICEPAYRU_S',null,null,$id_shop);
		$pdf->SetFontSize(9);
		$pdf->Write(6, $org_name, '', false, 'L', 1);
		...
		$pdf->Ln(18);
		$pdf->SetFontSize(14);
		if (Configuration::get('PS_INVOICE_NUMBER_ORDER', null, null, 1)) {
			$pdf->Write(0, 'СЧЕТ №'.$order->order_number.' '.'от '.date("d.m.Y",strtotime($order->invoice_date)).' г.', '', false, 'C');
		} else {
		...
		*/
		$pdf->MultiCell(35, 0, 'Поставщик: ', 0, 'L', false, 0);
//		$pdf->MultiCell(140, 0, ($result['org_name'] ? $result['org_name'] : $address_invoice->company).', ИНН '.$result['inn'].'/'.$result['kpp'].', ' . ($result['org_ur_addr'] ? $result['org_ur_addr'] : ' '.$address_invoice->city.', '.$address_invoice->address1), 0, 'L', 0, 1);
		$pdf->MultiCell(140, 0, $org_name, 0, 'L', 0, 1);
		$pdf->Ln(2);

		$pdf->MultiCell(35, 0, 'Плательщик: ', 0, 'L', false, 0);
//		$pdf->MultiCell(140, 0, ($result['org_name'] ? $result['org_name'] : $address_delivery->company).', ' . ($result['org_post_addr'] ? $result['org_post_addr'] : ' '.$address_delivery->city.', '.$address_delivery->address1), 0, 'L', 0, 0);
		$pdf->MultiCell(140, 0, ($result['org_name'] ? $result['org_name'] : $address_delivery->company), 0, 'L', false, 0);
		...
		if ($with_image == 1) {
//			$first_page_line_count = 30;
			$first_page_line_count = 27;
		} else {
			$first_page_line_count = 38;
		}
		$product_count = 0;
...
//		$first_page_otstup = 109;
		...
		{
			$pdf->lasth = 0;
			if ((($line_number == $first_page_line_count) && ($page_number == 1)) || (($line_number == $other_page_line_count))) {
				$pdf->AddPage();
				$page_number++;
				$line_number = 1;
...
//					$other_page_line_count = $product_count;
				...
				if (Configuration::get('PS_INVOICE_NUMBER_ORDER', null, null, 1)) {
					$pdf->MultiCell(150, 0, 'Счет №'.$order->order_number.' '.'от '.date("d.m.Y",strtotime($order->invoice_date)).' г.                                                       Страница '.$page_number, 0, 'L', 0, 1);
				} else {
					$pdf->MultiCell(150, 0, 'Счет №'.$order->id.' '.'от '.date("d.m.Y",strtotime($order->invoice_date)).' г.                                                       Страница '.$page_number, 0, 'L', 0, 1);
				}
				$pdf->Ln(3);
			}
			if ($product['product_quantity'] > 0) {
//				$pdf->MultiCell(80, $pdf->lasth, $product_count.' '.$line_number.' '.$product['reference'].' '.substr($product['product_name'],0,50), 1, 'L', 0, 0, 24, $pdf->GetY(), false, 0, false, true, $pdf->lasth, 'M', 1);
	...
	//			$pdf->MultiCell(18, $pdf->lasth, 'шт.', 1, 'C', 0, 0, 104, $pdf->GetY(), false, 0, false, true, $pdf->lasth, 'M', 1);
				$pdf->MultiCell(18, $pdf->lasth, Product::SGetProductUnity($product['sale_unity']), 1, 'C', 0, 0, 104, $pdf->GetY(), false, 0, false, true, $pdf->lasth, 'M', 1);
...
//				$totalWithTax += (((($product['product_price'] * $product['product_quantity']) / 100) * 18) + ($product['product_price'] * $product['product_quantity']));
				$totalWithTax += (($product['product_price'] * $product['product_quantity']));
...
//		$pdf->MultiCell(25, 4, number_format($order->total_paid_tax_incl - $order->total_paid_tax_excl, 2, '.', ''), 1, 'R', 0, 1);
		$pdf->MultiCell(25, 4, number_format($totalWithTax / 118 * 18, 2, '.', ''), 1, 'R', 0, 1);
		...
		$pdf->Ln(1);
		$pdf->SetFontSize(8);
		$pdf->MultiCell(150, 0, 'Всего наименований '.--$i.', на сумму '.number_format($totalWithTax, 2, '.', '').' руб.', 0, 'L', 0, 1);
...
//		$pdf->MultiCell(150, 0, mb_strtoupper(mb_substr($totalstr, 0, 1, 'utf-8'), 'utf-8').mb_substr($totalstr, 1, mb_strlen($totalstr, 'utf-8'), 'utf-8'), 0, 'L', 0, 1);
		...
		$pdf->MultiCell(150, 5, Configuration::get('INVOICEPAYRU_GD').' ________________________________________ '.Configuration::get('INVOICEPAYRU_FIO'), 0, 'L', 0, 0);
		if ($with_image == 1 && Configuration::get('INVOICEPAYRU_FP')) {
			if ($page_number == 1) {
				$pdf->Image(_PS_IMG_DIR_.'/'.Configuration::get('INVOICEPAYRU_FP'),5,$line_number*3.84+$first_page_otstup,165,65) ;
			} else {
				$pdf->Image(_PS_IMG_DIR_.'/'.Configuration::get('INVOICEPAYRU_FP'),17,$line_number*3.84+$other_page_otstup,120,45) ;
			}
		}
...
//		$pdf->MultiCell(130, 5, 'Главный бухгалтер ________________________________________ (ф.и.о)', 0, 'L', 0, 0);
		...
		$invoice_name = Configuration::get('INVOICEPAYRU_PREFIX');
//		$invoice_name = 'Schet_';

		if (Configuration::get('PS_INVOICE_NUMBER_ORDER', null, null, Shop::getContextShopID())) {
//			return $pdf->Output(Configuration::get('INVOICEPAYRU_PREFIX').sprintf('%06d', $order->order_number).'.pdf', $mode);
			return $pdf->Output($invoice_name.$order->order_number.'.pdf', $mode);
		} else {
//			return $pdf->Output(Configuration::get('INVOICEPAYRU_PREFIX').sprintf('%06d', $order->id).'.pdf', $mode);
			return $pdf->Output($invoice_name.$order->id.'.pdf', $mode);
		}
	}
