	...
	**/
	...
	this.initSlidersListView = function(){
		...
		jQuery('body').on('click', '.rs-reload-shop', function(){
			...
			UniteAdminRev.ajaxRequest("update_shop", {});
		...
		jQuery('body').on('click', '.template_slider_item_import .icon-install_slider', function(){
			...
			if($this.hasClass('deny_download')){
                                alert(rev_lang.this_template_requires_version+' '+$this.data('versionneed')+' '+rev_lang.of_slider_revolution);
				return false;
			}
...
//						ahref = '?page=revslider&view=slide&id='+ response.id;
						ahref = revsliderbackoffice+'&view=slide&id='+ response.id;
					}else{
