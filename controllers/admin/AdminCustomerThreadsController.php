...
class AdminCustomerThreadsControllerCore extends AdminController
{
    ...
    public function renderView()
    {
        ...
        if ($thread->id_customer) {
            ...
            $orders = Order::getCustomerOrders($customer->id, false, null, 'admin');
    ...
    public function getTimeline($messages, $id_order)
    {
        ...
        if (Validate::isLoadedObject($order)) {
            $order_history = $order->getHistory($this->context->language->id, false, false, 0, 'admin');
            foreach ($order_history as $history) {
                $link_order = $this->context->link->getAdminLink('AdminOrders').'&vieworder&id_order='.(int)$order->id;

                $content = '<a class="badge" target="_blank" href="'.Tools::safeOutput($link_order).'">'.$this->l('Order').' #'.(int)$order->order_number.'</a><br/><br/>';

                $content .= '<span>Статус: '.$history['ostate_name'].'</span>';
                $content .= '<br><span>Комментарий: '.$history['comment'].'</span>';
