...
class CustomerCore extends ObjectModel
{
    ...
    /** @var float Discount Р РѕСЃСЃРёСЏ */
    public $skid_2;

    /** @var float Discount РљРёС‚Р°Р№ */
    public $skid_1;
...
public $glob_skid;
	...
	/** Текущее состояние со скидкой или нет*/
    public $with_skid;

	/** Отображать цену без скидки (перечеркнутая) или нет*/
	public $show_price_wo_skid;
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'from_year' =>                    array('type' => self::TYPE_INT, 'validate' => 'isUnsignedId'),
            'own_folder' =>                    array('type' => self::TYPE_STRING, 'validate' => 'isString', 'size' => 255),
            'lastname' =>                    array('type' => self::TYPE_STRING, 'validate' => 'isName', 'required' => true, 'size' => 32),
            'firstname' =>                    array('type' => self::TYPE_STRING, 'validate' => 'isName', 'required' => true, 'size' => 32),
            'my_name' =>                    array('type' => self::TYPE_STRING, 'validate' => 'isString', 'size' => 32),
            'phone1' =>                    array('type' => self::TYPE_STRING, 'validate' => 'isString', 'size' => 12),
            'phone2' =>                    array('type' => self::TYPE_STRING, 'validate' => 'isString', 'size' => 12),
        ...
        if (!$id_lang || $id_lang == $ps_lang_default) {
            if (!in_array('required', $skip) && (!empty($data['required']) || in_array($field, $required_fields))) {
                if (Tools::isEmpty($value)) {
					if ($field == 'my_name') {
						if (Configuration::get('PS_OWN_NAME', null, null, Context::getContext()->shop->id)) {
							if ($human_errors) {
    ...
    {
        $sql = 'SELECT `id_customer`, `email`, `firstname`, `lastname`, `my_name`
    ...
    /**
     * Р’РѕР·РІСЂР°С‰Р°РµС‚ РќРђРРњР•РќРћР’РђРќРР• РїРѕР»СЊР·РѕРІР°С‚РµР»СЏ РІ СЃРёСЃС‚РµРјРµ РїРѕ СѓРєР°Р·Р°РЅРЅРѕРјСѓ С€Р°Р±Р»РѕРЅСѓ
     *
     * @param string $passwd Password
     * @return bool result
     */
    public static function getCustomerNameById($id_customer, $shablon = null)
    {
        if (!Validate::isUnsignedId($id_customer)) {
            die(Tools::displayError());
        }
        $cache_id = 'Customer::getCustomerNameById'.(int)$id_customer.'-'.$shablon;
        if (!Cache::isStored($cache_id)) {
			if ($shablon === null) {
				$shablon = 'CONCAT(LEFT(`firstname`, 1), \'. \', `lastname`) `customer`';
			}
    ...
    public static function searchByName($query, $limit = null)
    {
        ...
        $sql .= ' UNION ('.$sql_base.' WHERE `my_name` LIKE \'%'.pSQL($query).'%\' '.Shop::addSqlRestriction(Shop::SHARE_CUSTOMER).')';
//        $sql = '('.$sql_base.' WHERE 1)';
    ...
    public static function searchByEmployeeId($id_employee)
    {
        $sql = 'SELECT *
				FROM `'._DB_PREFIX_.'customer` WHERE `id_employee` = '.$id_employee;
     ...
     * @throws PrestaShopDatabaseException
     */
    public static function searchById($query, $limit = null)
    {
        $sql_base = 'SELECT *
				FROM `'._DB_PREFIX_.'customer`';
        $sql = '('.$sql_base.' WHERE `id_customer` = '.(int)$query.' '.Shop::addSqlRestriction(Shop::SHARE_CUSTOMER).')';
//        $sql = '('.$sql_base.' WHERE 1)';
    ...
    */
    ...
    public static function getIsndsCustomerByID($id_customer)
    {
        if (!Validate::isUnsignedId($id_customer)) {
            die(Tools::displayError());
        }
