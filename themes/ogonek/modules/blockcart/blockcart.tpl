...
<div class="clearfix__col-xs-12__col-sm-3__f-con {if $PS_CATALOG_MODE} header_user_catalog{/if}">
{/if}
*}
<!-- MODULE Block cart -->
	...
	<div class="shopping_cart">
		<a href="{$link->getPageLink($order_process, true)|escape:'html':'UTF-8'}" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">
            {assign var='num' value=count($products)}
			{*<b>{$num} {if $num==1}товар{elseif $num>1 && $num<5}товара{else}товаров{/if}</b>*}
			<span class="ajax_block_cart_total">{$total}</span>
			{*<b>{l s='Cart' mod='blockcart'}</b>*}
			<span class="ajax_cart_prod_num{if $cart_qties == 0} unvisible{/if}">{$num}</span>
...
<!--             <span class="unvisible">12345</span>
 -->			
...
{* 													{if $priceDisplay == $smarty.const.PS_TAX_EXC}
										...
										<span class="remove_link">
											{if !isset($customizedDatas.$productId.$productAttributeId) && (!isset($product.is_gift) || !$product.is_gift)}
												<a class="ajax_cart_block_remove_link" href="{$link->getPageLink('cart', true, NULL, "delete=1&id_product={$product.id_product|intval}&ipa={$product.id_product_attribute|intval}&id_address_delivery={$product.id_address_delivery|intval}&token={$static_token}")|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='remove this product from my cart' mod='blockcart'}">&nbsp;x</a>
											{/if}
										</span>
									</dt>
									{if isset($product.attributes_small)}
											...
											{foreach from=$customizedDatas.$productId.$productAttributeId[$product.id_address_delivery] key='id_customization' item='customization' name='customizations'}
												<li name="customization">
													<div data-id="deleteCustomizableProduct_{$id_customization|intval}_{$product.id_product|intval}_{$product.id_product_attribute|intval}_{$product.id_address_delivery|intval}" class="deleteCustomizableProduct">
														<a class="ajax_cart_block_remove_link" href="{$link->getPageLink('cart', true, NULL, "delete=1&id_product={$product.id_product|intval}&ipa={$product.id_product_attribute|intval}&id_customization={$id_customization|intval}&token={$static_token}")|escape:'html':'UTF-8'}" rel="nofollow">&nbsp;x</a>
													</div>
						...
						<p class="cart-buttons">
							<a id="button_order_cart" class="btn btn-default button button-small" href="{$link->getPageLink("$order_process", true)|escape:"html":"UTF-8"}" title="{l s='Check out' mod='blockcart'}" rel="nofollow">
								<span>
									{l s='ПЕРЕЙТИ В КОРЗИНУ' mod='blockcart'}<!-- <i class="icon-chevron-right right"></i> -->
								</span>
							</a>
						</p>
					</div>
