 ...
 */
class AdminRevizControllerCore extends AdminController
{
    /**
    ...
    public function __construct()
    {
        ...
        $this->className = 'Reviz';
        $this->identifier = 'id_reviz';
        $this->lang = false;
...
//        $this->addRowAction('updatereceipt');
        ...
        $this->fields_list = array(
            ...
            'sost_name' => array(
                'title' => $this->l('Состояние'),
                'filter_key' => 'rs!sost_name',
                'color' => 'color',
            ),
            'date_do' => array(
                'title' => $this->l('Проведена'),
                'align' => 'left',
                'type' => 'date',
                'havingFilter' => true,
                'filter_key' => 'a!date_do'
            ),
            'date_actual' => array(
                'title' => $this->l('Оформлена'),
                'align' => 'left',
                'type' => 'date',
                'havingFilter' => true,
                'filter_key' => 'a!date_actual'
            ),
            'sum_itog' => array(
                'title' => $this->l('Итоги'),
                'align' => 'left',
                'havingFilter' => true,
                'filter_key' => 'a!sum_itog'
            ),
            'correctno_name' => array(
                'title' => $this->l('Статус'),
                'filter_key' => 'rc!correctno_name',
                'color' => 'color',
            ),
        );
...
// Иванов 
		...
		$this->warehouses = Warehouse::getWarehouses(false,$this->id_shop);
		foreach ($this->warehouses as $items) {
...
// Иванов ^		
    ...
    public function init()
    {
        ...
        if (Tools::isSubmit('addreviz') ||
            Tools::isSubmit('submitAddreviz') ||
            (Tools::isSubmit('updatereviz') && Tools::isSubmit('id_reviz'))) {
    ...
    public function initPageHeaderToolbar()
    {
        if ($this->display == 'details') {
            $this->page_header_toolbar_btn['back'] = array(
                'href' => Context::getContext()->link->getAdminLink('AdminReviz'),
                'desc' => $this->l('Back to list', null, null, false),
                'icon' => 'process-icon-back'
            );
        } elseif (empty($this->display)) {
            $this->page_header_toolbar_btn['new_reviz'] = array(
                'href' => self::$currentIndex.'&addreviz&token='.$this->token,
                'desc' => $this->l('Создать новую ревизию', null, null, false),
    ...
    {
        if (Tools::isSubmit('addreviz') ||
            Tools::isSubmit('updatereviz') ||
            Tools::isSubmit('submitAddreviz') ||
            Tools::isSubmit('submitUpdatereviz')) {
            if (Tools::isSubmit('addreviz') ||    Tools::isSubmit('submitAddreviz')) {
                $this->toolbar_title = $this->l('Новая ревизия');
            }

            $update = false;
            if (Tools::isSubmit('updatereviz') || Tools::isSubmit('submitUpdatereviz')) {
                $this->toolbar_title = $this->l('Stock: Manage reviz');
                $update = true;
            }
            ...
            //get warehouses list
            $warehouses = Warehouse::getWarehouses(false,$this->id_shop);
            ...
            $this->fields_form = array(
                'legend' => array(
                    'title' => $this->l('Информация о ревизии'),
                    'icon' => 'icon-pencil'
                ),
                'input' => array(
                    array(
                        'type' => 'text',
                        'label' => $this->l('Название ревизии'),
                        'name' => 'reviz_name',
                        'required' => true,
                    ),
                    array(
					...
					array(
						...
						'default_value' => Context::getContext()->shop->id
					),
                ),
                'submit' => array('title' => $this->l('Сохранить')),
                'buttons' => 
                    array(
                        'save-and-stay' => array(
                            'title' => $this->l('Сохранить и остаться'),
                            'name' => 'submitAddrevizAndStay',
                            'type' => 'submit',
    ...
    public function renderList()
    {
        ...
        $this->_select = '
			...
			a.reviz_name,
			w.name AS warehouse,
			rc.correctno_name AS correctno_name,
			rs.sost_name AS sost_name,
			a.date_do,
			a.date_actual,
			a.sum_plus,
			a.sum_minus,
        ...
        $this->_join = '
			LEFT JOIN `'._DB_PREFIX_.'reviz_sost` rs ON
			(
				rs.id_reviz_sost = a.status_sost
			)
			LEFT JOIN `'._DB_PREFIX_.'reviz_correctno` rc ON
			(
				rc.id_reviz_correctno = a.status_correctno
			)
    ...
    {
        $id_reviz = (int)Tools::getValue('id_reviz', 0);
        ...
        $reviz = new Reviz($id_reviz);
//        $supply_order_state = new SupplyOrderState($supply_order->id_supply_order_state);

//        if (!Validate::isLoadedObject($reviz) || !Validate::isLoadedObject($supply_order_state)) {
        if (!Validate::isLoadedObject($reviz) ) {
            $this->errors[] = Tools::displayError('The specified reviz is not valid');
            return parent::initContent();
        }
    ...
    public function postProcess()
    {
        ...
        if (Tools::isSubmit('submitAddreviz') || Tools::isSubmit('submitAddrevizAndStay')) {
            $this->action = 'save';
    ...
    protected function renderCSV()
    {
        ...
        if (Tools::isSubmit('csv_orders')) {
            ...
            if ($id_warehouse != -1) {
				...
				$orders->where('id_warehouse', 'IN', $this->wh_list);
			}
            $orders->getAll();
        ...
        elseif (Tools::isSubmit('csv_orders_details')) {
            ...
            foreach ($ids as $id) {
                ...
                if ($id_warehouse != -1) {
					...
					$query->where('so.id_warehouse IN ('.$this->wh_list.')');
				}
