        ...
        foreach ($menu_items as $item) {
            ...
            switch (substr($item, 0, strlen($value[1]))) {
                ...
                case 'CMS_CAT':
                    $category = new CMSCategory((int)$id, (int)$id_lang);
                    if (count($category)) {
                        $uri = "/content/category/{$category->id}-{$category->link_rewrite}";
                        $this->_menu .= sprintf(
                            '<li class="sf-menu__item">
                                <a class="sf-menu__link%s" href="%s" title="%s" data-link="'.$category->getLink().'">%s</a>',
                            ($uri == $_SERVER['REQUEST_URI']) ? ' sf-menu__link--active': '',
                            Tools::HtmlEntitiesUTF8($uri),
                            $category->name,
                            $category->name
                        );
