        ...
        <div class="revolution-template-switcher">
            ...
            if (!empty($author_template_slider) && is_array($author_template_slider)) {
                foreach ($author_template_slider as $name => $v) {
                    ...
                    ?>" class="revolution-templatebutton"><?php echo($name);
    ...
    <div class="revolution-basic-templates revolution-template-groups">
        ...
        if (!empty($tp_template_slider)) {
            foreach ($tp_template_slider as $m_slider) {
                $is_supported = Tools::version_compare($m_slider['required'], RevSliderGlobals::SLIDER_REVISION, 'le');
                if ($m_slider['cat'] != 'Revolution Base' || !$is_supported) {
                    continue;
                }
                ...
                if (!@RevsliderPrestashop::getIsset($m_slider['installed']) && !@RevsliderPrestashop::getIsset($m_slider['is_new'])) {
                    ...
                    if (!empty($c_slides)) {
                        ...
                        <div class="template_group_wrappers <?php
                                 ...
                                 foreach ($c_slides as $c_slide) {
                                     ...
                                     if (@RevsliderPrestashop::getIsset($m_slider['preview'])) {
                                     ...
                                     $tmpl->RevSliderTemplate($c_slide, $c_slider->getID()); //add the Slider ID as we want to add a Slider and no Slide
                    ...
                    <div class="template_group_wrappers not-imported-wrapper <?php
                             ...
                             ?>">
                    <?php
                    $tmpl->writeImportTemplateMarkup($m_slider); //add the Slider ID as we want to add a Slider and no Slide
...
?>
        ...
        <div style="clear:both;width:100%"></div>
         ...
         if (!empty($author_template_slider) && is_array($author_template_slider)) {
            ...
            <div class="revolution-<?php echo sanitize_title($name);

                 ?> revolution-template-groups" style="display: none">
                 <?php
                 if (!empty($v)) {
