...
class SearchCore
{
        ...
        $order_way = 'desc', $ajax = false, $use_cookie = true, Context $context = null)
    {
        ...
        if ($ajax) {
            $sql = 'SELECT DISTINCT p.id_product, pl.name pname, cl.name cname,
					...
					WHERE p.`id_product` '.$product_pool.' AND stock.quantity > 0
            ...
            foreach ($weight_array as $key => $weight) {
                if ((int)$weight) {
                    switch ($key) {
                        ...
                        break;
                        case 'pa_ean13_2':
                            $sql .= ', pa.ean13_2 AS pa_ean13_2';
                        break;
                        case 'pa_ean13_3':
                            $sql .= ', pa.ean13_3 AS pa_ean13_3';
                        break;
                        case 'pa_upc':
    ...
    public static function indexation($full = false, $id_product = false)
    {
        ...
        $weight_array = array(
            ...
            'ean13_2' => Configuration::get('PS_SEARCH_WEIGHT_REF'),
            'pa_ean13_2' => Configuration::get('PS_SEARCH_WEIGHT_REF'),
            'ean13_3' => Configuration::get('PS_SEARCH_WEIGHT_REF'),
            'pa_ean13_3' => Configuration::get('PS_SEARCH_WEIGHT_REF'),
