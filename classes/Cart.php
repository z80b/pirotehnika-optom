    ...
    public function getCartRules($filter = CartRule::FILTER_ACTION_ALL)
    {
        ...
        if (!Cache::isStored($cache_key)) {
            $result = Db::getInstance()->executeS('
				...
				WHERE `id_cart` = '.(int)$this->id.' AND my_id_shop = '.Context::getContext()->shop->id.'
				'.($filter == CartRule::FILTER_ACTION_SHIPPING ? 'AND free_shipping = 1' : '').'
				'.($filter == CartRule::FILTER_ACTION_ONLY_THIS_SHOP ? 'AND my_id_shop = '.Context::getContext()->shop->id : '').'
    ...
    public function getProducts($refresh = false, $id_product = false, $id_country = null)
    {
       ...
       // Product cache must be strictly compared to NULL, or else an empty cart will add dozens of queries
        ...
        $sql->select('cp.`id_product_attribute`, cp.`id_product`, cp.`quantity` AS cart_quantity, cp.id_shop, pl.`name`, p.`is_virtual`,
						...
						stock.`quantity` AS quantity_available, p.`width`, p.`country_prois`, p.`max_discount`, p.`height`, p.`depth`, stock.`out_of_stock`, p.`weight`,
        ...
        foreach ($result as &$row) {
            ...
            if (isset($CatId_Summ)) {
                $CatId_Summ[$row['id_category_default']]+=$row['price'];//maxim
            }
		...
		if (Configuration::get('PS_IS_SHOW_DISCOUNT',null,null,(int)$this->id_shop) && $context->customer->show_skid) {
			if ($row['country_prois'] == 1) {
				...
				$row['price_with_reduction'] = round($row['price_with_reduction'] * (100 - MIN($context->customer->skid_2, $row['max_discount'])) / 100, 2);
            ...
            switch (Configuration::get('PS_ROUND_TYPE')) {
                case Order::ROUND_TOTAL:
                    ...
                    $row['total_wt'] = $row['price_without_reduction'] * (int)$row['cart_quantity'];
                    break;
                case Order::ROUND_LINE:
                    $row['total'] = Tools::ps_round($row['price_with_reduction_without_tax'] * (int)$row['cart_quantity'], _PS_PRICE_COMPUTE_PRECISION_);
                    $row['total_wt'] = Tools::ps_round($row['price_without_reduction'] * (int)$row['cart_quantity'], _PS_PRICE_COMPUTE_PRECISION_);
                    break;

                case Order::ROUND_ITEM:
                default:
                    $row['total'] = Tools::ps_round($row['price_with_reduction_without_tax'], _PS_PRICE_COMPUTE_PRECISION_) * (int)$row['cart_quantity'];
                    $row['total_wt'] = Tools::ps_round($row['price_without_reduction'], _PS_PRICE_COMPUTE_PRECISION_) * (int)$row['cart_quantity'];
                    break;
            }

            $row['price_wt'] = $row['price_without_reduction'];
			...
			$row['SummByCatId'] = isset($CatId_Summ) ? $CatId_Summ : 0;
			//$row['id_category_default'] = $CatId_Summ;
...
//        $context = Context::getContext();
		...
		$order_total_without_disc = 0;
		$order_total_retail_price = 0;
		...
		if (Configuration::get('PS_IS_SHOW_DISCOUNT',null,null,(int)$virtual_context->shop->id) && $virtual_context->customer->show_skid) {
			if ($product['country_prois'] == 1) {
				...
				$price = round($price * (100 - MIN($virtual_context->customer->skid_2, $product['max_discount'])) / 100, 2);
		...
		@$this->categ_sum[$product['category']] += $price;
            ...
            switch ($ps_round_type) {
                case Order::ROUND_TOTAL:
					...
					$order_total_without_disc += $price_without_disc * (int)$product['cart_quantity'];
					$order_total_retail_price += $retail_price * (int)$product['cart_quantity'];
                    break;
                ...
                case Order::ROUND_LINE:
                    ...
                    $order_total_without_disc += Tools::ps_round($price_without_disc * $product['cart_quantity'], $compute_precision);
                    $order_total_retail_price += Tools::ps_round($retail_price * $product['cart_quantity'], $compute_precision);
					break;
                ...
                default:
                    ...
                    $order_total_without_disc += Tools::ps_round($price_without_disc, $compute_precision) * (int)$product['cart_quantity'];
                    $order_total_retail_price += Tools::ps_round($retail_price, $compute_precision) * (int)$product['cart_quantity'];
                    break;
            }
        }
		...
		if ($type == Cart::ONLY_PRODUCTS && $with_taxes == true) {
			return Tools::ps_round((float)$order_total_without_disc, $compute_precision);
		} else {
			if ($type == Cart::RETAIL_PRICES) {
				return Tools::ps_round((float)$order_total_retail_price, $compute_precision);
			} else {		
    ...
    public function getCategoriesSum($id_category) {
        if (!isset($this->categ_sum)) return 0;
    	return $this->categ_sum[$id_category];
    }
    ...
    */
    ...
    /**
    * Return cart volume
    * @return float Cart volume
    */
    public function getTotalVolume($products = null)
    {
		$order_volume = Db::getInstance()->getValue('
		SELECT SUM(p.`width` * p.`height` * p.`depth` * cp.`quantity` / 1000000) as nb
		FROM `'._DB_PREFIX_.'cart_product` cp
		LEFT JOIN `'._DB_PREFIX_.'product` p ON (cp.`id_product` = p.`id_product`)
		WHERE cp.`id_cart` = '.(int)$this->id);

        return round((float)$order_volume, 6);
    }
    ...
    */
    public function getSummaryDetails($id_lang = null, $refresh = false)
    {
        ...
        $gift_products = array();
        $cart_rules = $this->getCartRules(CartRule::FILTER_ACTION_ONLY_THIS_SHOP);
    ...
    public function checkQuantities($return_product = false)
    {
        ...
        foreach ($this->getProducts() as $product) {
			...
			$productObject = new Product((int)$product['id_product'], true, (int)Context::getContext()->language->id);	
			if (!$productObject->checkQty($product['cart_quantity'])) {
				return $return_product ? $product : false;
			}
        }
