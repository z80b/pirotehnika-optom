...
<!-- 	<td style="display:none;">
		<div class="row">
			<div class="input-group fixed-width-xl">
				<div class="input-group-addon">Прайс</div>
				{if $currency->format % 2}<div class="input-group-addon">{$currency->sign} </div>{/if}
				<input type="text" name="add_product[original_product_price]" id="add_product_original_product_price" value="" disabled="disabled" />
				{if !($currency->format % 2)}<div class="input-group-addon">{$currency->sign} </div>{/if}
			</div>
		</div>
	</td>
 -->	
	<td style="display:none;" id="add_product_original_product_price">{displayPrice price=0 currency=$currency->id}</td>
	<td style="display:none;">
		<div class="row">
			<div class="input-group fixed-width-xl">
				<div class="input-group-addon">{l s='Price'}</div>
				{if $currency->format % 2}<div class="input-group-addon">{$currency->sign} </div>{/if}
				<input type="text" name="add_product[product_price_tax_incl]" id="add_product_product_price_tax_incl" value="" disabled="disabled" />
				{if !($currency->format % 2)}<div class="input-group-addon">{$currency->sign} </div>{/if}
				<input type="hidden" name="add_product[product_price_tax_excl]" id="add_product_product_price_tax_excl" value="" disabled="disabled" />
			</div>
		</div>
	</td>

	<td style="display:none;" class="productQuantity" >
		<input type="number" min="1" class="form-control fixed-width-sm" name="add_product[product_quantity]" id="add_product_product_quantity" value="1" disabled="disabled" />
	</td>
	...
	<td style="display:none;" colspan="2">
		...
		<select class="form-control {if ($PS_INVOICE_NUMBER_ORDER)}hidden{/if}" name="add_product[invoice]" id="add_product_product_invoice" disabled="disabled">
