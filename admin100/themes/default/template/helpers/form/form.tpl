...
$(function () {
	var admin_customers_tab_link = "index.php?controller=AdminCustomers&token=85e7f01f7f80f8999512cfa938f912df" ;
	$("#cust_name").autocomplete("index.php?controller=AdminCustomers&token=85e7f01f7f80f8999512cfa938f912df",
		{
			minChars: 3,
			max: 10,
			width: 500,
...
{assign var='my_value_text' value=1}
						...
						{else}
							{block name="label"}
								{if isset($input.label)}
									<label class="control-label col-lg-3{if isset($input.required) && $input.required && $input.type != 'radio'} required{/if} {if isset($input.class)}{$input.class|escape:'html':'utf-8'}{/if}">
										{if isset($input.hint)}
										...
										<select name="{$input.name|escape:'html':'utf-8'}"
											...
											{else}
												{foreach $input.options.query AS $option}
													...
													{else}
														<option value="{$option[$input.options.id]}" 
{if isset($input.my_tip2)}
	{if ((($my_value_text == 2) && ($option[$input.options.id] < 5)) || (($my_value_text == 1) && ($option[$input.options.id] > 4)))}
		class="hidden"
	{/if}
{/if}
															{if isset($input.multiple)}
															...
															{else}
																{if $fields_value[$input.name] == $option[$input.options.id]}
																	selected="selected" 
{if isset($input.my_tip1)}
	{assign var='my_value_text' value=$fields_value[$input.name]}
{/if}
																{/if}
															{/if}
														>{$option[$input.options.name]}</option>
...
<input type="text"
	name="cust_name"
	id="cust_name"
	value="{customer::getCustomerNameById($value_text,'CONCAT(`my_name`, \' -  \',`firstname`, \' \', `lastname`) `customer`')}"
/>
			<div id="customers88">987987987</div>

									{/if}
								{elseif $input.type == 'distribute_amounts'}
<div class="distribute_amounts">
	<button type="button" class="btn btn-default" id="todo_distribute_one_amount" >
		<i class="icon-ok text-success"></i>
		{l s='Распределить'}
	</button>
<script type="text/javascript">
$(document).ready(function() {
	$("#todo_distribute_one_amount").unbind('click').click(function() {
		$('.distribute_one_amount').show();
	});	
});
</script>
	<div class="distribute_one_amount" id="distribute_one_amount" style="display: none;">
