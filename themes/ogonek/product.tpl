		...
		<div class="ps-product-page__block">
			...
			<form id="buy_block"{if $PS_CATALOG_MODE && !isset($groups) && $product->quantity > 0} class="hidden"{/if} action="{$link->getPageLink('cart')|escape:'html':'UTF-8'}" method="post">
				...
				<div class="box-info-product">
					...
					<div class="product_attributes text-center clearfix col-xs-5">
						...
						<p id="quantity_wanted_p"{if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || $PS_CATALOG_MODE} style="display: block;"{/if}>
							...
							<input type="number"
								   ...
								   class="ajax_input_prod_{$product->id} ajax_input_prod_to_null text"
								   value="{if isset($productsCart.cart_quantity)}{$productsCart.cart_quantity}{else}0{/if}"
								   {*onchange="changeProductCountInCart({$product->id}, 'ajax_input_prod_{$product->id}'); yaCounter46713966.reachGoal('ADDCART_TAB'); return true;"*}
							/>
					...
					<div class="box-cart-bottom col-xs-7">
						<div{if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || (isset($restricted_country_mode) && $restricted_country_mode) || $PS_CATALOG_MODE} class="unvisible"{/if}>
							<p id="fancy_add_to_cart" class="text-center buttons_bottom_block no-print">
								<button type="submit"
										name="Submit"
										class="exclusive"
								...
								>{l s='В ЗАКАЗ'}
									<!-- <span>{if $content_only && (isset($product->customization_required) && $product->customization_required)}{l s='Customize'}{else}{l s='ДОБАВИТЬ В КОРЗИНУ'}{/if}</span> -->
								</button>
							</p>
						</div>
