...
class AuthControllerCore extends FrontController
{
    ...
    protected function processSubmitAccount()
    {
        ...
        // Preparing customer
        $customer = new Customer();
        $customer->my_name = (empty(Configuration::get('PS_OWN_NAME_DEFAULT', null, null, Context::getContext()->shop->id)) ? Tools::getValue('customer_lastname').' '.Tools::getValue('customer_firstname') : Configuration::get('PS_OWN_NAME_DEFAULT', null, null, Context::getContext()->shop->id));
        ...
        if (!Configuration::get('PS_REGISTRATION_PROCESS_TYPE') && !$this->ajax && !Tools::isSubmit('submitGuestAccount')) {
            if (!count($this->errors)) {
				...
				$is_easy_reg = !Configuration::get('PS_HARD_REGISTRATION', null, null, Context::getContext()->shop->id);
				if ($is_easy_reg == 1) {
					$customer->active = 1;
				} else {
					$is_easy_code_reg = Configuration::get('PS_EASY_CODE_REGISTRATION', null, null, Context::getContext()->shop->id);
					if ($is_easy_code_reg == 0) {
						$customer->active = 0;
					} else {
						$is_easy_code_reg_promo = Configuration::get('PS_EASY_CODE_REGISTRATION_PROMO', null, null, Context::getContext()->shop->id);
						$is_easy_code_reg_client = Tools::getValue('my_create_promo');
						if ($is_easy_code_reg_promo == $is_easy_code_reg_client) {
							$customer->active = 1;
						} else {
							$customer->active = 0;
						}	
    ...
    protected function sendConfirmationMail(Customer $customer)
    {
        ...
        Mail::Send(
            ...
            Configuration::get('PS_MAIL_USER', null, null, Context::getContext()->shop->id),
