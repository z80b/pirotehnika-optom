...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('#submitSavePayment').on('click',function(){
			...
			saveMyPayment($('#id_my_payments_tip').val(),$('#id_my_payments').val(), $('#id_my_payments_vid option:selected').val());
//			updateQty($('#id_product').val(), $('#ipa_'+$('#id_product').val()+' option:selected').val(), 0, $('#qty').val());
		});

	function saveMyPayment(id_new_payments_tip, id_new_payment, id_new_payment_vid)
	{
		$.ajax({
			type:"POST",
