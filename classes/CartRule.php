...
class CartRuleCore extends ObjectModel
{
    ...
    const FILTER_ACTION_ONLY_THIS_SHOP = 6;
...
// Иванов 
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'my_id_shop' =>            array('type' => self::TYPE_INT, 'validate' => 'isUnsignedId'),
        ),
    );
    ...
    protected function getCartRuleCombinations($offset = null, $limit = null, $search = '')
    {
		...
		AND  cr.my_id_shop = '.(int)Context::getContext()->shop->id.'
		AND (
		...
		WHERE cr.cart_rule_restriction = 1 AND cr.my_id_shop = '.(int)Context::getContext()->shop->id.'
    ...
    public function getAssociatedRestrictions($type, $active_only, $i18n, $offset = null, $limit = null, $search_cart_rule_name = '')
    {
        ...
        if (!Validate::isLoadedObject($this) || $this->{$type.'_restriction'} == 0) {
			...
			'.($type == 'cart_rule' ? 'AND t.id_cart_rule != '.(int)$this->id.' AND t.my_id_shop = '.(int)Context::getContext()->shop->id : '').
            $shop_list.
    ...
    public static function getCartsRuleByCode($name, $id_lang, $extended = false)
    {
        $sql_base = 'SELECT cr.*, crl.*
						...
						LEFT JOIN '._DB_PREFIX_.'cart_rule_lang crl ON (cr.id_cart_rule = crl.id_cart_rule AND crl.id_lang = '.(int)$id_lang.')';
        if ($extended) {
            return Db::getInstance()->executeS('('.$sql_base.' WHERE my_id_shop = '.Context::getContext()->shop->id.' and code LIKE \'%'.pSQL($name).'%\') UNION ('.$sql_base.' WHERE my_id_shop = '.Context::getContext()->shop->id.' and name LIKE \'%'.pSQL($name).'%\')');
        } else {
            return Db::getInstance()->executeS($sql_base.' WHERE my_id_shop = '.Context::getContext()->shop->id.' and code LIKE \'%'.pSQL($name).'%\'');
        }
    }
}
