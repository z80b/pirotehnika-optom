...
{assign var=my_product_price value=$product['original_product_price']}
...
<!-- 			{if $product.product_supplier_reference}{l s='Supplier reference:'} {$product.product_supplier_reference}{/if}
 -->		
	...
	<td class="unit_price_tax_excl">
		{if $my_view}
			<!-- Цена Прайс -->
			<span>{displayPrice price=$my_product_price currency=$currency->id}</span>
		{else}
			<!-- Цена без Налогов -->
	...
	{if $my_view}
		<!-- Скидка прайс -->
		<td class="unit_price_tax_excl">
			<span {if $product.my_real_discount <> $product.my_price_discount} class="badge"{/if}>{number_format( $product.my_price_discount, 3, '.', '' )}&nbsp;%</span>
		</td>
		<!-- Скидка реальная -->
		<td class="unit_price_tax_excl">
			<span {if ($product.my_real_discount < $product.my_price_discount)} class="badge badge-success" {elseif ($product.my_real_discount > $product.my_price_discount)} class="badge badge-danger" {/if}>{number_format( $product.my_real_discount, 3, '.', '' )}&nbsp;%</span>
		</td>
	{/if}
	<td>
		<!-- Цена с НДС -->
		...
		<div class="product_price_edit" style="display:none;">
			...
			<div class="form-group">
				{if $my_view}
					<!-- Скидка реальная  видимая -->
					<div class="fixed-width-xl">
						<div class="input-group">
							<p>Скидка %</p>
							<input type="text" name="product_my_discount" class="edit_product_my_discount" value="{number_format($product.my_real_discount, 3, '.', '' )}"/>
						</div>
					</div>
				{/if}
					<!-- Цена без Налогов видимая-->
					<div class="fixed-width-xl">
						<div class="input-group">
							...
							{if !($currency->format % 2)}<div class="input-group-addon">{$currency->sign} {l s='t ax excl.'}</div>{/if}
						</div>
					</div>
					<!-- Цена с НДС -->
					<div class="fixed-width-xl">
						<div class="input-group">
							<input type="hidden" name="product_price_price" class="edit_product_price_price"  value="{$my_product_price}"</>
							...
							{if !($currency->format % 2)}<div class="input-group-addon">{$currency->sign} {l s='ta x incl.'}</div>{/if}
						</div>
					</div>
			</div>
		</div>
	...
	<td class="productQuantity text-center">
		<span id="old_product_qty" class="product_quantity_show">{(int)$product['product_quantity'] - (int)$product['customized_product_quantity']}</span>
		{if $can_edit}
		<span class="product_quantity_edit" style="display:none;">
			<input type="number"  min="1" name="product_quantity" class="edit_product_quantity" value="{$product['product_quantity']|htmlentities}"/>
		</span>
		{/if}
	</td>
	...
	{if ($can_edit && !$order->hasBeenShipped())}
