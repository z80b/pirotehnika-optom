...
<script type="text/javascript">
	...
	var employees_list = new Array();
	{foreach from=$employees_list2 key='id_employee' item='name'}
		employees_list['{$id_employee}'] = '{$name}';
	{/foreach}
	...
	$(document).ready(function() {
		...
		$('#id_all_employee').change(function() {
				searchCustomers(); 
			});
		$('#product').typeWatch({
			captureLength: 1,
...
//			var id_order_state = defaults_order_state[this.value];
		...
		$("#voucher").autocomplete('{$link->getAdminLink('AdminCartRules')|addslashes}', {
					...
					extraParams: {
						...
						id_new_employee: $('#id_new_employee').val()
					}
				}
			)
		...
		$('#customer_part').on('click','button.setup-customer',function(e){
			e.preventDefault();
			$('#id_new_employee').val($('#id_customer_employee').val());
	...
	function searchCustomers()
	{
		$.ajax({
			...
			data : {
				...
				customer_employee: $('#id_all_employee').val()},
			success : function(res)
			{
				if(res.found)
				{
					...
					$.each(res.customers, function() {
						...
						if (is_show_fio) {
							...
							html += '<div class="panel-heading">'+this.my_name;
						}
						html += '<span class="pull-right">#'+this.id_customer+'</span></div>';
						if (this.id_employee > 0) {
							html += '<span>'+this.email+'</span><span class="pull-right">  Менеджер - '+employees_list[this.id_employee]+'</span><br/>';
							html += '<input id="id_customer_employee" class="hidden" value="'+this.id_employee+'"></span><br/>';
						} else {
							html += '<span>'+this.email+'</span><span class="pull-right">  Менеджер - Не закреплен</span><br/>';
							html += '<input id="id_customer_employee" class="hidden" value="0"></span><br/>';
						}	
						...
						html += '<div id="employeeCard" class="employeeCard col-lg-4">';
						html += 	'<div class="panel">';
						html += 		'<div class="panel-heading">'
						html += 			'<span>Заказ будет закреплен за этим менеджером:</span>'
						html +=			'</div>';
						html += 		'<select id="id_new_employee" value="$order->id_employee" class="chosen form-control fixed-width-lg" name="id_new_employee">';
						html += 			'{foreach from=$employees_list item=state}';
						html += 				'<option value="{$state['id_employee']|intval}">{$state['name']|escape}</option>';
						html += 			'{/foreach}';
						html += 		'</select>';
						html += 	'</div>';
						html += '</div>';
					...
					<div class="col-lg-6">
						...
						<a class="fancybox_customer btn btn-default" href="{$link->getAdminLink('AdminCustomers')|escape:'html':'UTF-8'}&amp;addcustomer&amp;liteDisplaying=1&amp;submitFormAjax=1#">
							...
							{l s='Add new customer'}
								...
								<div class="col-lg-6">
									<select id="id_all_employee" value="$order->id_employee" class="chosen form-control fixed-width-lg" name="id_all_employee">
									{foreach from=$employees_list item=state}
										<option value="{$state['id_employee']|intval}">{$state['name']|escape}</option>
									{/foreach}
									</select>
								</div>
					</div>
					...
					<div class="col-lg-9">
						<select name="id_order_state" id="id_order_state">
							{foreach from=$order_states item='order_state'}
								<option value="{$order_state.id_order_state}" {if (isset($smarty.post.id_order_state) && $order_state.id_order_state == $smarty.post.id_order_state) || ($order_state.id_order_state == 20)}selected="selected"{/if}>{$order_state.name}</option>
							{/foreach}
						</select>
					</div>
				</div>
