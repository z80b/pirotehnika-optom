...
class RevSliderNavigation
{
    ...
    public function initById($nav_id)
    {
        ...
        $row = $wpdb->getRow($wpdb->prepare("SELECT `id`, `handle`, `type`, `css`, `settings` FROM " . $wpdb->prefix . RevSliderGlobals::TABLE_NAVIGATION_NAME . " WHERE `id` = %d", $nav_id), ARRAY_A);
    ...
    public static function getAllNavigationsShort()
    {
        ...
        $navigations = $wpdb->getResults("SELECT `id`, `handle`, `name` FROM " . $wpdb->prefix . RevSliderGlobals::TABLE_NAVIGATION_NAME, ARRAY_A);
    ...
    public static function getAllNavigations($defaults = true, $raw = false)
    {
        ...
        $navigations = $wpdb->getResults("SELECT * FROM " . $wpdb->prefix . RevSliderGlobals::TABLE_NAVIGATION_NAME, ARRAY_A);

        if ($raw == false) {
            foreach ($navigations as $key => $nav) {
                $navigations[$key]['css'] = preg_replace_callback('/\\"(.*)?\\"\;/', create_function('$m', 'return "\"{$m[1]}\"";'), Tools::jsonDecode($navigations[$key]['css'], true));
                $navigations[$key]['markup'] = Tools::jsonDecode($navigations[$key]['markup'], true);
                if (@RevsliderPrestashop::getIsset($navigations[$key]['settings'])) {
                    $navigations[$key]['settings'] = Tools::jsonDecode($navigations[$key]['settings'], true);
                }
            }
        }
            ...
            if (!empty($def)) {
                if ($raw == false) {
                    foreach ($def as $key => $nav) {
                        $def[$key]['css'] = preg_replace_callback('/\\"(.*)?\\"\;/', create_function('$m', 'return "\"{$m[1]}\"";'), RevSliderBase::stripslashesDeep(Tools::jsonDecode($def[$key]['css'], true)));
                        $def[$key]['markup'] = RevSliderBase::stripslashesDeep(Tools::jsonDecode($def[$key]['markup'], true));
                    }
                    if (@RevsliderPrestashop::getIsset($def[$key]['settings'])) {
                        $def[$key]['settings'] = RevSliderBase::stripslashesDeep(Tools::jsonDecode($def[$key]['settings'], true));
                    }
                }
    ...
    public function createUpdateFullNavigation($data)
    {
        if (!empty($data) && is_array($data)) {
            ...
            foreach ($data as $vals) {
                ...
                if (!@RevsliderPrestashop::getIsset($vals['markup']) || !@RevsliderPrestashop::getIsset($vals['css'])) {
                    continue;
                }
                if (@RevsliderPrestashop::getIsset($vals['default']) && $vals['default'] == true) {
                    continue; //defaults can't be deleted
                }

                if (@RevsliderPrestashop::getIsset($vals['id'])) { //new will be added temporary to navs to tell here that they are new
                    if ((int) ($vals['id']) === 0) {
                        continue;
                    } //0
    ...
    public static function createUpdateNavigation($data, $nav_id = 0)
    {
        ...
        if (@RevsliderPrestashop::getIsset($data['default']) && $data['default'] == true) {
            return false;
        }

        if (!@RevsliderPrestashop::getIsset($data['settings'])) {
            $data['settings'] = '';
        }

        if ($nav_id > 0) {
            $response = $wpdb->update(
                $wpdb->prefix . RevSliderGlobals::TABLE_NAVIGATION_NAME,
                array(
                    'name' => $data['name'],
            ...
            $response = $wpdb->insert($wpdb->prefix . RevSliderGlobals::TABLE_NAVIGATION_NAME, array('name' => $data['name'], 'handle' => sanitize_title($data['name']), 'css' => Tools::jsonEncode($data['css']), 'markup' => Tools::jsonEncode($data['markup']), 'settings' => Tools::jsonEncode($data['settings'])));
        }
    ...
    public function deleteNavigation($nav_id = 0)
    {
        ...
        if (!@RevsliderPrestashop::getIsset($nav_id) || (int) ($nav_id) == 0) {
            return __('Invalid ID', 'revslider');
        }


        $response = $wpdb->query("DELETE FROM `" . $wpdb->prefix . RevSliderGlobals::TABLE_NAVIGATION_NAME . "` WHERE `id`='" . $nav_id . "';");
        if ($response === false) {
            return __('Navigation could not be deleted', 'revslider');
        }
            ...
            foreach ($nav_handle as $handle => $u) {
                foreach ($navs as $n => $v) {
                    if ($v['handle'] == $handle) {
                        ...
                        $entries[$handle]['css'] = str_replace("\\\\", "\\\\\\\\", $entries[$handle]['css']);
                        $entries[$handle]['css'] = str_replace("\\\"", "\\\\\\\"", $entries[$handle]['css']);
                        break;
                    }
                }
