...
*/
include_once(_PS_MODULE_DIR_.'invoicepayru/invoice.php');
        ...
        if (Validate::isLoadedObject($this->context->cart) && $this->context->cart->OrderExists() == false) {
            ...
            foreach ($package_list as $id_address => $packageByAddress) {
                foreach ($packageByAddress as $id_package => $package) {
                    ...
                    $order->rperiod = Configuration::get('PS_ORDER_RPERIOD', null, null, (int)$this->context->shop->id);
                    $order->id_employee = (int)$this->context->cart->id_employee;
//                    $order->id_employee = 6;
                    $order->id_customer = (int)$this->context->cart->id_customer;
                    $order->id_trc = Trc::getCustomerTrcByID($order->id_customer);
                    $order->isnds = Customer::getIsndsCustomerByID($order->id_customer);
					if ($order->isnds == 1) {
						$order->sfnom = Order::getNextSfnom();
					} else {
						$order->sfnom = '';
					}
					...
					$order->order_number = Order::getLastOrderNumber() + 1;
...
//                    $order->total_products_wt = (float)$this->context->cart->getOrderTotal(true, Cart::ONLY_PRODUCTS, $order->product_list, $id_carrier);
                    ...
                    $order->total_products_wt = (float)$this->context->cart->getOrderTotal(true, Cart::RETAIL_PRICES, $order->product_list, $id_carrier);
...
//                    $order->total_paid_tax_incl = (float)Tools::ps_round((float)$this->context->cart->getOrderTotal(true, Cart::BOTH, $order->product_list, $id_carrier), _PS_PRICE_COMPUTE_PRECISION_);
                    $order->total_paid_tax_excl = (float)Tools::ps_round((float)$this->context->cart->getOrderTotal(true, Cart::ONLY_PRODUCTS, $order->product_list, $id_carrier), _PS_PRICE_COMPUTE_PRECISION_);
                    $order->total_paid_tax_incl = (float)Tools::ps_round((float)$this->context->cart->getOrderTotal(true, Cart::ONLY_PRODUCTS, $order->product_list, $id_carrier), _PS_PRICE_COMPUTE_PRECISION_);
					...
					$result2 = Order::setNextSfnom($order->sfnom + 1);
            ...
            foreach ($order_detail_list as $key => $order_detail) {
                ...
                if (!$order_creation_failed && isset($order->id)) {
                    ...
                    if ($id_order_state != Configuration::get('PS_OS_ERROR') && $id_order_state != Configuration::get('PS_OS_CANCELED') && $this->context->customer->id) {
                        ...
                        '{order_name}' => $order->getUniqReference(),
                        '{date}' => Tools::displayDate(date('d-m-y H:i:s'), null, 1),
 			...
 			if (Configuration::get('PS_RUS_INVOICE_FORM', null, null, Shop::getContextShopID())) {
//				$file_attachement['content'] = SPDF::invoice(Tools::getValue('id_order'),'S');
				$file_attachement['invoice'] = SPDF::invoice($order->id,'S');
				if (Configuration::get('PS_INVOICE_NUMBER_ORDER', null, null, Shop::getContextShopID())) {
					$file_attachement['name'] = Configuration::get('INVOICEPAYRU_PREFIX', null, null, $order->id_shop).sprintf('%06d', $order->order_number).'.pdf';
				} else {
					$file_attachement['name'] = Configuration::get('INVOICEPAYRU_PREFIX', null, null, $order->id_shop).sprintf('%06d', $order->invoice_number).'.pdf';
				}
			} else {
...
// удалено чтобы не приходило клиенту письмо о создании заказа
/* 						Mail::Send(
                                (int)$order->id_lang,
                                'order_conf',
