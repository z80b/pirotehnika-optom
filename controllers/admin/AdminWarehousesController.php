...
// Иванов 
    ...
    public function renderForm()
    {
        ...
        $query = new DbQuery();
        $query->select('e.id_employee, CONCAT(e.lastname," ",e.firstname) as name');
        $query->from('employee', 'e');
        $query->leftJoin('employee_shop', 'es', 'es.id_employee = e.id_employee');
        $query->where('e.active = 1');
        $query->where('es.id_shop = '.$this->id_shop);
