...
{block name="override_tpl"}
	...
	var admin_my_cust_tab_link = "{$link->getAdminLink('AdminCustomers')|addslashes}";
		...
		<div class="col-lg-7">
			<div class="panel">
				<div class="panel-heading">
					...
					<span class="badge">{l s="#"}{$order->order_number}</span>
					<span class="badge">Менеджер - {$order_employee}</span>
					<div class="panel-heading-action">
						...
						<form action="{$currentIndex|escape:'html':'UTF-8'}&amp;vieworder&amp;token={$smarty.get.token}" method="post" class="form-horizontal well hidden-print">
							<div class="row">
								<div class="col-lg-7">
									<div id="comment" >
										Комментарий к заказу<input type="text" name="Comment" value="{$order->comment}" />
									</div>
									<input type="hidden" name="id_order" value="{$order->id}" />
								</div>
								<div class="col-lg-2">
									<button type="submit" name="submitComment" class="btn btn-primary">
										{l s='Сохранить комментарий'}
									</button>
								</div>
							</div>
						</form>
						<form action="{$currentIndex|escape:'html':'UTF-8'}&amp;vieworder&amp;token={$smarty.get.token}" method="post" class="form-horizontal well hidden-print">
							<div class="row">
								<div class="col-lg-7">
									<select id="id_new_employee" value="$order->id_employee" class="chosen form-control fixed-width-lg" name="id_new_employee">
									{foreach from=$employees_list item=state}
										<option value="{$state['id_employee']|intval}"{if $state['id_employee'] == $order->id_employee} selected="selected" disabled="disabled"{/if}>{$state['name']|escape}</option>
									{/foreach}
									</select>
									<input type="hidden" name="id_order" value="{$order->id}" />
								</div>
								<div class="col-lg-1">
									<button type="submit" name="submitEmployee" class="btn btn-primary">
										{l s='Закрепить за менеджером'}
									</button>
								</div>
							</div>
						</form>
...
$('document').ready(function(){
	$('#isnds').click(function() {
		if ($('#nomnds').hasClass('hidden') == true ) {
			$('#nomnds').removeClass('hidden').show();
		} else {
			$('#nomnds').addClass('hidden').show();
		}
	});
});
</script>
							<div class="row">
								<div class="col-lg-7">
									<div class="checkbox">
										<label for="isnds">
											<input type="checkbox" name="isnds" id="isnds" value="1" {if $order->isnds}checked="checked"{/if} >
											{l s='Заказ с НДС'}</label>
									</div>
									<div id="nomnds" class="{if !$order->isnds}hidden{/if}">
										№ счета-фактуры<input type="text" name="sfnom" value="{$order->sfnom}" />
										<div class="input-group ">
											Дата счета-фактуры<input type="text" name="sfdate" class="datepicker" value="{$order->sfdate}" />
								...
								<div class="col-lg-2">
									<button type="submit" name="submitNDS" class="btn btn-primary">
										{l s='Сохранить сведения о НДС'}
									</button>
								</div>
							</div>
						</form>
								...
								<tbody>
									{foreach from=$history item=row key=key}
										...
										<tr>
											...
											<td colspan=4>
												<input type="text" name="id_order" value="{$row['comment']|stripslashes}" />
											</td>
										</tr>
									{/foreach}
								</tbody>
						...
						<form action="{$currentIndex|escape:'html':'UTF-8'}&amp;vieworder&amp;token={$smarty.get.token}" method="post" class="form-horizontal well hidden-print">
							<div class="row">
								<div class="col-lg-9">
									...
									<input type="hidden" name="id_order" value="{$order->id}" />
									<input type="text" name="state_comment" value="" />
								</div>
								<div class="col-lg-3">
						...
						<table class="table">
							<thead>
								<tr>
									...
									<th><span class="title_box ">{l s='Сумма'}</span></th>
							...
							<tbody>
								...
								<tr class="payment_information" style="display: none;">
									<td colspan="5">
										...
										<hr>
										<input type="hidden" class="my_payment_id" value="{$payment->id}"/>
										<div >
												<p>
												<b>{l s='Date'}</b>
												<div class="input-group ">
													<input type="text" name="my_payment_date{$payment->id}" class="my_payment_date" value="{$payment->date_add}" />
													<div class="input-group-addon">
														<i class="icon-calendar-o"></i>
													</div>
												</div>
												...
												<p>
												<b>{l s='Payment method'}</b>
												<div class="input-group ">
													<select name="my_payment_method{$payment->id}" class="my_payment_metod" value="{$payment->payment_method|escape:'html':'UTF-8'}">
														<option {if ($payment->payment_method == "")} selected="selected"{/if}></option>
													{foreach from=$payment_methods item=payment_method}
														<option {if ($payment->payment_method == $payment_method)} selected="selected"{/if}>{$payment_method}</option>
													{/foreach}
													</select>
												</div>
												</p>
												<p>
												<b>{l s='Transaction ID'}</b>
												<div class="input-group ">
												{if $my_view}
													<select name="my_payment_transaction_id{$payment->id}" class="my_payment_transaction_id" value="{$payment->transaction_id|escape:'html':'UTF-8'}"/>
														<option {if ($payment->transaction_id == "")} selected="selected"{/if}></option>
													{foreach from=$payment_kasses item=payment_kass}
														<option {if ($payment->transaction_id == $payment_kass)} selected="selected"{/if}>{$payment_kass}</option>
													{/foreach}
													</select>
												{else}	
													<input type="text" name="payment_transaction_id{$payment->id}" value="{$payment->transaction_id|escape:'html':'UTF-8'}" class="form-control fixed-width-sm"/>
												{/if}
												...
												<b>{l s='Сумма'}</b>
												<div class="input-group ">
													<input type="text" name="my_payment_amount{$payment->id}" class="edit_old_payment_sum" value="{$payment->amount}"/>
													<select name="my_payment_currency{$payment->id}" class="my_payment_currency form-control fixed-width-xs pull-left" style="display: none" >
												...
												<div class="input-group ">
													{if count($invoices_collection) > 0}
														<select name="my_payment_invoice{$payment->id}" id="payment_invoice" class="my_payment_invoice" value="
															{if $invoice = $payment->getOrderInvoice($order->id)}
																{$invoice->getInvoiceNumberFormatted($current_id_lang, $order->id_shop)}
															{else}
															{/if}
									...
									<td class="actions">
										<div class="hidden">
												<button id="but_submitMySavePayment" class="btn btn-primary my_save_payment_information" type="submit" name="submitMySavePayment">
													{l s='Save'}
												</button>
												<br>
												<br>
										...
										<button id="but_submitMyRemovePayment" class="btn btn-primary  my_delete_payment_information" type="submit" name="submitMyRemovePayment">
											&nbsp;&nbsp;&nbsp;{l s='Remove'}&nbsp;&nbsp;&nbsp;
										</button>
												<br>
												<br>
												...
												<br>
												<br>
										<button class="btn btn-primary my_cancel_payment_information" >
											&nbsp;&nbsp;&nbsp;{l s='Cancel'}&nbsp;&nbsp;&nbsp;
										</button>
									</td>
								...
								<tr class="current-edit hidden-print">
									...
									<td>
										<select name="payment_method" class="edit_payment_metod">
											<option> </option>
										{foreach from=$payment_methods item=payment_method}
									...
									<td>
										{if $my_view}
											<select name="payment_transaction_id" class="edit_payment_transaction_id" />
												<option> </option>
											{foreach from=$payment_kasses item=payment_kass}
												<option>{$payment_kass}</option>
											{/foreach}
											</select>
										{else}	
									...
									<td>
										<input type="text" name="payment_amount" value="" class="edit_new_payment_sum" />
										<select name="payment_currency" class="payment_currency form-control fixed-width-xs pull-left" style="display: none" >
											{foreach from=$currencies item=current_currency}
												<option value="{$current_currency['id_currency']}"{if $current_currency['id_currency'] == $currency->id} selected="selected"{/if}>{$current_currency['sign']}</option>
											{/foreach}
										</select>
									...
									<td class="actions">
										<button id="but_submitAddPayment" class="btn btn-primary" type="submit" name="submitAddPayment">
											{l s='Add'}
										</button>
									</td>
								</tr>
			...
			<div class="panel">
				{if $customer->id}
					<div class="panel-heading">
						...
						<span class="badge">Менеджер - {$customer_employee}</span>
					</div>
					<div class="row">
						<div class="col-xs-6">
							{if ($customer->isGuest())}
				...
				<div class="row">
					...
					<div class="tab-content panel">
						...
						<form action="{$currentIndex|escape:'html':'UTF-8'}&amp;vieworder&amp;token={$smarty.get.token}" method="post" class="form-horizontal well hidden-print">
							<div class="row">
								<div class="col-lg-7">
									<select id="id_new_trc" value="$order->id_trc" class="chosen form-control fixed-width-lg" name="id_new_trc">
									{foreach from=$trcs_list item=state}
										<option value="{$state['id_trc']|intval}"{if $state['id_trc'] == $order->id_trc} selected="selected" disabled="disabled"{/if}>{$state['name']|escape}</option>
									{/foreach}
									</select>
									<input type="hidden" name="id_order" value="{$order->id}" />
								</div>
								<div class="col-lg-1">
									<button type="submit" name="submitTrc" class="btn btn-primary">
										{l s='Изменить ТК'}
									</button>
								</div>
							</div>
							...
							<thead>
								<tr>
									...
									<th>
										...
										{if $my_view}
											<small class="text-muted">по прайсу</small>
										{else}	
									...
									{if $my_view}
									<th>
										<span class="title_box ">Скидка</span>
										<small class="text-muted">карточка</small>
									</th>
									<th>
										<span class="title_box ">Скидка</span>
										<small class="text-muted">в заказе</small>
									</th>
									{/if}
									<th>
										<span class="title_box ">{l s='Unit Price'}</span>
						...
						<table class="table" id="orderProducts">
							<thead>
								<tr>
									...
									<th><span class="title_box ">В карточке</span></th>
									<th><span class="title_box ">Скидка</span></th>
									<th></th>
									<th><span class="title_box ">В заказе</span></th>
								</tr>
							</thead>
							<tbody>
								<!-- Китай --> 
								<tr>
									...
									<td></td>
									<td></td>
									<td><span class="title_box ">Китай</span></td>
									<td class="actions">
										<button id="but_SetCartKit" class="btn btn-primary" type="submit" name="submitSetCartKit">
											Сохранить в карточке - >
										</button>
									</td>
									<td class="unit_price_tax_excl">
										<input type="text" name="customer_skid_kit" class="edit_customer_skid_kit" value="{number_format($customer->skid_1, 3, '.', '' )}"/>
									</td>
									<td><span class="title_box ">Китай</span></td>
									<td class="actions">
										<button id="but_SetSkidKitNull" class="btn btn-primary" type="button" name="submitSetSkidKitNull">
											Обнулить - >
										</button>
									</td>
									<td>
										<input type="text" name="order_skid_kit" class="edit_order_skid_kit" value="{number_format($order->skid_1, 3, '.', '' )}"/>
									</td>
								</tr>
								<!-- Россия --> 
								<tr>
									...
									<td></td>
									<td></td>
									<td><span class="title_box ">Россия</span></td>
									<td class="actions">
										<button id="but_SetCartRos" class="btn btn-primary" type="submit" name="submitCartRos">
											Сохранить в карточке - >
										</button>
									</td>
									<td class="unit_price_tax_excl">
										<input type="text" name="customer_skid_ros" class="edit_customer_skid_ros" value="{number_format($customer->skid_2, 3, '.', '' )}"/>
									</td>
									<td><span class="title_box ">Россия</span></td>
									<td class="actions">
										<button id="but_SetSkidRosNull" class="btn btn-primary" type="submit" name="submitSetSkidRosNull">
											Обнулить - >
										</button>
									</td>
									<td>
										<input type="text" name="order_skid_ros" class="edit_order_skid_ros" value="{number_format($order->skid_2, 3, '.', '' )}"/>
									</td>
								</tr>
								<!-- Польша --> 
								<tr>
									...
									<td></td>
									<td></td>
									<td><span class="title_box ">Польша</span></td>
									<td class="actions">
										<button id="but_SetCartPol" class="btn btn-primary" type="submit" name="submitCartPol">
											Сохранить в карточке - >
										</button>
									</td>
									<td class="unit_price_tax_excl">
										<input type="text" name="customer_skid_pol" class="edit_customer_skid_pol" value="{number_format($customer->skid_3, 3, '.', '' )}"/>
									</td>
									<td><span class="title_box ">Польша</span></td>
									<td class="actions">
										<button id="but_SetSkidPolNull" class="btn btn-primary" type="submit" name="submitSetSkidPolNull">
											Обнулить - >
										</button>
									</td>
									<td>
								...
								<tr>
									...
									<td></td>
									<td class="actions">
										<button id="but_getSkidFromCart" class="btn btn-primary" type="submit" name="submitCartPol">
											Использовать скидки из карточки
										</button>
									</td>
									<td class="actions">
										<button id="but_submitAllSkid" class="btn btn-primary" type="submit" name="submitSetSkidPolNull">
											Применить скидки
										</button>
									</td>
								</tr>
