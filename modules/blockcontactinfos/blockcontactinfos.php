...
class Blockcontactinfos extends Module
{
	...
	public function install()
	{
		...
		return (parent::install() && $this->registerHook('header') && $this->registerHook('footer') && $this->registerHook('top'));
	}
	...
	{	
		if (!$this->isCached('blockcontactinfos_top.tpl', $this->getCacheId()))
			foreach (Blockcontactinfos::$contact_fields as $field)
				$this->smarty->assign(strtolower($field), Configuration::get($field));
		return $this->display(__FILE__, 'blockcontactinfos_top.tpl', $this->getCacheId());
	}
