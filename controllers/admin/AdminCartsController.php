...
class AdminCartsControllerCore extends AdminController
{
	...
	public function __construct()
    {
        ...
        $this->_select = 'CONCAT(LEFT(c.`firstname`, 1), \'. \', c.`lastname`) `customer`, c.`my_name` `my_customer_name`, a.id_cart total, ca.name carrier,
		CONCAT(e.`lastname`, \'. \', LEFT(e.`firstname`, 1)) AS `employee`,
		IF (IFNULL(o.order_number, \''.$this->l('Non ordered').'\') = \''.$this->l('Non ordered').'\', IF(TIME_TO_SEC(TIMEDIFF(\''.pSQL(date('Y-m-d H:i:00', time())).'\', a.`date_add`)) > 86400, \''.$this->l('Abandoned cart').'\', \''.$this->l('Non ordered').'\'), o.order_number) AS status, IF(o.order_number, 1, 0) badge_success, IF(o.order_number, 0, 1) badge_danger, IF(co.id_guest, 1, 0) id_guest';
        ...
        $this->fields_list = array(
 		...
 		if ($this->is_show_fio == true) {
			...
			$this->fields_list = array_merge($this->fields_list, array(
				'my_customer_name' => array(
					'title' => $this->l('Наименование'),
					'filter_key' => 'c!my_name'
				),
				'customer' => array(
					'title' => $this->l('Customer'),
					'filter_key' => 'c!lastname'
 ...
 */			
    ...
    public function renderView()
    {
        ...
        $this->tpl_view_vars = array(
            'kpi' => $kpi,
            'products' => $products,
            'discounts' => $cart->getCartRules(CartRule::FILTER_ACTION_ONLY_THIS_SHOP),
            'order' => $order,
            'cart' => $cart,
            'currency' => $currency,
            'customer' => $customer,
    ...
    public function ajaxPreProcess()
    {
        if ($this->tabAccess['edit'] === '1') {
			...
			$this->context->cart->id_employee = (int)Tools::getValue('id_new_employee');
//$text66 = '  ajax  - '.'search yes - '.$this->context->cart->id_employee.'*' ;
//file_put_contents('somefile.txt', PHP_EOL.$text66, FILE_APPEND);
    ...
    public function displayAjaxSearchCarts()
    {
        ...
        $orders = Order::getCustomerOrders((int)$id_customer, false, null, 'admin');
        $customer = new Customer((int)$id_customer);
