...
class OrderPaymentCore extends ObjectModel
{
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'id_my_payments' =>        array('type' => self::TYPE_INT, 'validate' => 'isUnsignedId'),
    ...
    /**
     * Get возвращает список разнесенных платежей 
     * @param int $order_reference
     * @return array
     * @since 1.5.0.13
     */
    public static function getByMyPaymetsId($id_my_payment)
    {
         return Db::getInstance()->executeS('
			SELECT o.order_number, o.id_customer, o.total_paid, op.amount, op.id_order_payment, o.id_order
			FROM `'._DB_PREFIX_.'order_payment` op
			LEFT JOIN `'._DB_PREFIX_.'orders` o ON (o.`reference` = op.`order_reference`)
			WHERE op.`id_my_payments` = '.$id_my_payment);

/* 		return ObjectModel::hydrateCollection('OrderPayment',
            Db::getInstance()->executeS('
			SELECT o.order_number, o.id_customer, o.total_paid, op.amount
			FROM `'._DB_PREFIX_.'order_payment` op
			LEFT JOIN `'._DB_PREFIX_.'orders` o ON (o.`reference` = op.`order_reference`)
			WHERE op.`id_my_payments` = '.$id_my_payment)
        );
