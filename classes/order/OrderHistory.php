        ...
        if (isset($result['template']) && Validate::isEmail($result['email'])) {
            ...
            if (Validate::isLoadedObject($order)) {
 			...
 			if (Configuration::get('PS_RUS_INVOICE_FORM', null, null, Shop::getContextShopID())) {
				$file_attachement['invoice']['content'] = SPDF::invoice($order->id,'S');
				if (Configuration::get('PS_INVOICE_NUMBER_ORDER', null, null, Shop::getContextShopID())) {
					$file_attachement['invoice']['name'] = Configuration::get('INVOICEPAYRU_PREFIX', null, null, $order->id_shop).sprintf('%06d', $order->order_number).'.pdf';
				} else {
					$file_attachement['invoice']['name'] = Configuration::get('INVOICEPAYRU_PREFIX', null, null, $order->id_shop).sprintf('%06d', $order->invoice_number).'.pdf';
				}
			} else {
