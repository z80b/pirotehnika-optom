...
function init()
{
			...
			parse: function(data) {
				...
				if (typeof(data.products) != 'undefined')
					for (var i = 0; i < data.products.length; i++)
						products[i] = { data: data.products[i], value: data.products[i].reference+' '+data.products[i].name };
				return products;
			},
			extraParams: {
				ajax: true,
	...
	.result(function(event, data, formatted) {
		...
		else
		{
			...
			$('#add_product_product_price_tax_incl').val(data.original_product_price);
			$('#add_product_product_price_tax_excl').val(data.original_product_price);
//			$('#add_product_original_product_price').val(data.original_product_price);
			$('#add_product_original_product_price').html(formatCurrency(data.original_product_price, currency_format, currency_sign, currency_blank));
	...
	$('input#add_product_product_quantity').unbind('mouseup').mouseup(function() {
		if (stock_management)
		{
			var quantity = parseInt($(this).val());
			if (quantity < 1 || isNaN(quantity))
	...
	$('.edit_new_payment_sum').unbind('keyup').keyup(function() {
		var pr_char = $(this).val();
		if (pr_char.charAt(pr_char.length-1) == '.') {
			return true;
		}
	...
	$('.edit_old_payment_sum').unbind('keyup').keyup(function() {
		var pr_char = $(this).val();
		if (pr_char.charAt(pr_char.length-1) == '.') {
			return true;
		}
	...
	$('button.submitProductChange').unbind('click').click(function(e) {
		...
		if (confirm(txt_confirm))
		{
			...
			$.ajax({
				...
				success : function(data)
				{
					...
					{	
						element2.closest('tr.product-line-row').find('td .edit_product_quantity').val(data.new_qty);
						jAlert(data.error);
					}
				}
			});
     ...
     /* Сохранить скидку Китай */	
	$('#but_SetCartKit').unbind('click').click(function(e) {

		if (confirm(txt_confirm))
		{
			var customer_skid_kit = parseFloat($('.edit_customer_skid_kit').val());
			if (isNaN(customer_skid_kit))
				customer_skid_kit = 0;
			query = 'ajax=1&token='+token+'&action=SetCustomerDiscountKit&customer_skid_kit='+customer_skid_kit+'&id_customer='+id_customer;
	...
	$('#but_SetCartGlob').unbind('click').click(function(e) {

		if (confirm(txt_confirm))
		{
			var customer_skid_glob = parseFloat($('.edit_customer_skid_glob').val());
			if (isNaN(customer_skid_glob))
				customer_skid_glob = 0;
			query = 'ajax=1&token='+token+'&action=SetCustomerDiscountGlob&customer_skid_glob='+customer_skid_glob+'&id_customer='+id_customer;
     ...
     /* Сохранить скидку Польша */	
	$('#but_SetCartPol').unbind('click').click(function(e) {

		if (confirm(txt_confirm))
		{
			var customer_skid_pol = parseFloat($('.edit_customer_skid_pol').val());
			if (isNaN(customer_skid_pol))
				customer_skid_pol = 0;
			query = 'ajax=1&token='+token+'&action=SetCustomerDiscountPol&customer_skid_pol='+customer_skid_pol+'&id_customer='+id_customer;
     ...
     /* Сохранить скидку Россия */	
	$('#but_SetCartRos').unbind('click').click(function(e) {

		if (confirm(txt_confirm))
		{
			var customer_skid_ros = parseFloat($('.edit_customer_skid_ros').val());
			if (isNaN(customer_skid_ros))
				customer_skid_ros = 0;
			query = 'ajax=1&token='+token+'&action=SetCustomerDiscountRos&customer_skid_ros='+customer_skid_ros+'&id_customer='+id_customer;
	...
	$('.edit_product_price_tax_excl').unbind('keyup').keyup(function() {
		...
		if (price_tax_excl < 0 || isNaN(price_tax_excl))
			price_tax_excl = 0;
		var tax_rate = current_product.tax_rate / 100 + 1;
//		var product_price_price = parseFloat($('.edit_product_price_price').val());
		var product_price_price = parseFloat($(this).parent().parent().parent().find('.edit_product_price_price').val());
		var new_discount_price = ps_round(price_tax_excl * tax_rate, 2);
		var discount_rate = ps_round((product_price_price-new_discount_price) / product_price_price * 100,6);
		$('.edit_product_my_discount:visible').val(discount_rate);
		$('.edit_product_price_tax_incl:visible').val(new_discount_price);
		// Update total product
		editProductRefreshTotal($(this));
	});
	$('.edit_product_my_discount').unbind('keyup').keyup(function() {
		var pr_char = $(this).val();
		if (pr_char.charAt(pr_char.length-1) == '.') {
			return true;
		}
		...
		var discount_percent = parseFloat($(this).val());
		if (isNaN(discount_percent))
			discount_percent = 0;
		var product_price_price = parseFloat($(this).parent().parent().parent().find('.edit_product_price_price').val());
		var new_discount_price = product_price_price * (1 - discount_percent / 100);
		var tax_rate = current_product.tax_rate / 100 + 1;
		var price_tax_incl = new_discount_price;
		$('.edit_product_price_tax_excl:visible').val(ps_round(price_tax_incl / tax_rate, 2));
		$('.edit_product_price_tax_incl:visible').val(ps_round(new_discount_price, 2));
		// Update total product
		editProductRefreshTotal($(this));
	});
	...
	$('.edit_product_price_tax_incl').keyup(function() {
		...
		if (price_tax_incl < 0 || isNaN(price_tax_incl))
			price_tax_incl = 0;
		$('.edit_product_price_tax_incl').val(price_tax_incl);
//		var product_price_price = parseFloat($('.edit_product_price_price').val());
		var product_price_price = parseFloat($(this).parent().parent().parent().find('.edit_product_price_price').val());
		var discount_rate = ps_round((product_price_price-price_tax_incl) / product_price_price * 100,6);
		var tax_rate = current_product.tax_rate / 100 + 1;
		$('.edit_product_price_tax_excl:visible').val(ps_round(price_tax_incl / tax_rate, 2));
		$('.edit_product_my_discount:visible').val(discount_rate);
		// Update total product
	...
	$('.edit_product_quantity').unbind('mouseup');
	$('.edit_product_quantity').mouseup(function() {
		var quantity = parseInt($(this).val());
		if (quantity < 1 || isNaN(quantity))
			quantity = 1;
		var stock_available = parseInt($(this).parent().parent().parent().find('td.product_stock').html());
	...
	// удаляет все товары в заказе
	$('#delete_all_products').unbind('click').click(function(e) {
		if (!confirm(txt_confirm))
			return false;
//		var tr_product = $(this).closest('.product-line-row');
//		var id_order_detail = $(this).closest('.product-line-row').find('td .edit_product_id_order_detail').val();
		var query = 'ajax=1&action=DeleteProductAllProductsFromOrder&token='+token+'&id_order='+id_order;
	...
	$('#but_submitAddPayment').unbind('click').click(function(e) {
		var my_paid_met = $('.edit_payment_metod').val();
 		if (my_paid_met.length == 1 ) {
			$('.edit_payment_metod').css('borderColor', my_newcolor);
			e.preventDefault();
			return false;
		}
		var my_paid_met = $('.edit_payment_transaction_id').val();
 		if (my_paid_met.length == 1 ) {
			$('.edit_payment_transaction_id').css('borderColor', my_newcolor);
			e.preventDefault();
			return false;
		}
		var my_paid_sum = parseFloat($('.edit_new_payment_sum').val());
		if (my_paid_sum <= 0 || isNaN(my_paid_sum)) {
			$('.edit_new_payment_sum').css('borderColor', my_newcolor);
			e.preventDefault();
			return false;
		}
//		e.preventDefault();
	...
	$('.edit_payment_metod').unbind('click').click(function(e) {
		$(this).css('borderColor', my_oldcolor);
		e.preventDefault();
	});

	$('.edit_new_payment_sum').unbind('click').click(function(e) {
		$(this).css('borderColor', my_oldcolor);
		e.preventDefault();
	});
	$('.edit_payment_transaction_id').unbind('click').click(function(e) {
		$(this).css('borderColor', my_oldcolor);
		e.preventDefault();
	});
	
	$('#but_SetSkidKitNull').unbind('click').click(function(e) {
		$('.edit_order_skid_kit').val(0.000);
		$('#but_submitAllSkid').css('backgroundColor', my_newcolor);
		e.preventDefault();
	});

	$('#but_SetSkidRosNull').unbind('click').click(function(e) {
		$('.edit_order_skid_ros').val(0.000);
		$('#but_submitAllSkid').css('backgroundColor', my_newcolor);
		e.preventDefault();
	});

	$('#but_SetSkidPolNull').unbind('click').click(function(e) {
		$('.edit_order_skid_pol').val(0.000);
		$('#but_submitAllSkid').css('backgroundColor', my_newcolor);
		e.preventDefault();
	});
$('#but_SetSkidGlobNull').unbind('click').click(function(e) {
		$('.edit_order_skid_glob').val(0.000);
		$('#but_submitAllSkid').css('backgroundColor', my_newcolor);
		e.preventDefault();
	});
	
	$('.edit_order_skid_glob').unbind('keyup');
	$('.edit_order_skid_glob').keyup(function() {
		$('#but_submitAllSkid').css('backgroundColor', my_newcolor);
		var pr_char = $(this).val();
		if (pr_char.charAt(pr_char.length-1) == '.') {
			return true;
	...
	$('.edit_order_skid_kit').unbind('keyup');
	$('.edit_order_skid_kit').keyup(function() {
		$('#but_submitAllSkid').css('backgroundColor', my_newcolor);
		var pr_char = $(this).val();
		if (pr_char.charAt(pr_char.length-1) == '.') {
			return true;
	...
	$('.edit_order_skid_ros').unbind('keyup');
	$('.edit_order_skid_ros').keyup(function() {
		$('#but_submitAllSkid').css('backgroundColor', my_newcolor);
		var pr_char = $(this).val();
		if (pr_char.charAt(pr_char.length-1) == '.') {
			return true;
	...
	$('.edit_order_skid_pol').unbind('keyup');
	$('.edit_order_skid_pol').keyup(function() {
		$('#but_submitAllSkid').css('backgroundColor', my_newcolor);
		var pr_char = $(this).val();
		if (pr_char.charAt(pr_char.length-1) == '.') {
			return true;
	...
	/* 	$('#but_SetSkidPolNull').unbind('click').click(function(e) {
		...
		var newcolor = '#fc0707'
		var oldcolor = '#040506';
		$('#but_submitAllSkid').css('backgroundColor', newcolor);
		setTimeout(function () {
			$("#but_submitAllSkid").css("backgroundColor", newcolor);
			setInterval(function () {
				$("#but_submitAllSkid").css("backgroundColor", flag? newcolor:oldcolor);
				flag = !flag;
			}, 300)
		}, 100);
		e.preventDefault();
 ...
 */	
	$('#but_getSkidFromCart').unbind('click').click(function(e) {
		var my_skid_kit = parseFloat($('.edit_customer_skid_kit').val());
		if (isNaN(my_skid_kit))
			my_skid_kit = 0;
		$('.edit_order_skid_kit').val(my_skid_kit);
		
		var my_skid_glob = parseFloat($('.edit_customer_skid_glob').val());
		if (isNaN(my_skid_glob))
			my_skid_glob = 0;
		$('.edit_order_skid_glob').val(my_skid_glob);
		
		var my_skid_ros = parseFloat($('.edit_customer_skid_ros').val());
		if (isNaN(my_skid_ros))
			my_skid_ros = 0;
		$('.edit_order_skid_ros').val(my_skid_ros);
		
		var my_skid_pol = parseFloat($('.edit_customer_skid_pol').val());
		if (isNaN(my_skid_pol))
			my_skid_pol = 0;
		$('.edit_order_skid_pol').val(my_skid_pol);
		$('#but_submitAllSkid').css('backgroundColor', my_newcolor);
		e.preventDefault();
	});

	/* Устанавливает скидку в заказе */
	$('#but_submitAllSkid').unbind('click').click(function(e) {
		$('#but_submitAllSkid').css('backgroundColor', my_oldcolor);
		e.preventDefault();
		if (confirm(txt_confirm))
		{
			var order_skid_kit = parseFloat($('.edit_order_skid_kit').val());
			if (isNaN(order_skid_kit))
				order_skid_kit = 0;
			
			var order_skid_glob = parseFloat($('.edit_order_skid_glob').val());
			if (isNaN(order_skid_glob))
				order_skid_glob = 0;
			
			var order_skid_ros = parseFloat($('.edit_order_skid_ros').val());
			if (isNaN(order_skid_ros))
				order_skid_ros = 0;
			var order_skid_pol = parseFloat($('.edit_order_skid_pol').val());
			if (isNaN(order_skid_pol))
				order_skid_pol = 0;
			query = 'ajax=1&token='+token+'&action=SetOrderDiscountAll&order_skid_kit='+order_skid_kit+'&order_skid_ros='+order_skid_ros+'&order_skid_glob='+order_skid_glob+'&order_skid_pol='+order_skid_pol+'&id_order='+id_order+'&id_customer='+id_customer;
	...
	$('.my_cancel_payment_information').unbind('click').click(function(e) {
			$(this).parent().parent().hide();
		e.preventDefault();
	});

	$('.my_save_payment_information').unbind('click').click(function(e) {

		if (confirm(txt_confirm))
		{
			var parent_element = $(this).parent().parent()	
			var my_payment_id = parseInt(parent_element.find('.my_payment_id').val());
			var my_payment_date = parent_element.find('.my_payment_date').val();
			var my_payment_metod = parent_element.find('.my_payment_metod').val();
			var my_payment_transaction_id = parent_element.find('.my_payment_transaction_id').val();
			var my_payment_sum = parseFloat(parent_element.find('.edit_old_payment_sum').val());
			var my_payment_currency = parent_element.find('.my_payment_currency').val();
			var my_payment_invoice = parent_element.find('.my_payment_invoice').val();
			query = 'ajax=1&token='+token+'&action=submitMySavePayment&id_order='+id_order+'&my_payment_id='+my_payment_id+'&my_payment_date='+my_payment_date+'&my_payment_metod='+my_payment_metod+'&my_payment_transaction_id='+my_payment_transaction_id+'&my_payment_sum='+my_payment_sum+'&my_payment_currency='+my_payment_currency+'&my_payment_invoice='+my_payment_invoice;
	...
	$('.my_delete_payment_information').unbind('click').click(function(e) {
		...
		if (confirm(txt_confirm))
		{
			...
			var my_payment_currency = parent_element.find('.my_payment_currency').val();
			var my_payment_invoice = parent_element.find('.my_payment_invoice').val();
			query = 'ajax=1&token='+token+'&action=submitMyRemovePayment&id_order='+id_order+'&my_payment_id='+my_payment_id+'&my_payment_date='+my_payment_date+'&my_payment_metod='+my_payment_metod+'&my_payment_transaction_id='+my_payment_transaction_id+'&my_payment_sum='+my_payment_sum+'&my_payment_currency='+my_payment_currency+'&my_payment_invoice='+my_payment_invoice;
