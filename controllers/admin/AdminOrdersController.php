...
//    protected $statuses_groups_array = array();
	protected $tax_nds = 1;
	public $is_show_fio;
...
//        $this->required_fields = array('blocker');
		...
		if ($this->is_show_fio == true) {
			...
			$this->_select .= ' c.`my_name` AS `customer`,';
		}
		$this->_select .= ' osl.`name` AS `osname`,
		CONCAT(e.`lastname`, \'. \', LEFT(e.`firstname`, 1)) AS `employee`,
		os.`order_state_groups` ,
		os.`color`,
		trc.`name` AS `trc`,
		IF((SELECT so.id_order FROM `'._DB_PREFIX_.'orders` so WHERE so.id_customer = a.id_customer AND so.id_order < a.id_order LIMIT 1) > 0, 0, 1) as new,
		country_lang.name as cname, 
		IF(((a.total_paid_real = 0) OR (a.total_paid < a.total_paid_real)) , 1, 0) badge_danger, IF((a.total_paid > a.total_paid_real) AND (a.total_paid_real <> 0) , 1, 0) badge_warning,
		...
		LEFT JOIN `'._DB_PREFIX_.'trc` trc ON (trc.`id_trc` = a.`id_trc`)
		LEFT JOIN `'._DB_PREFIX_.'order_state_group` osg ON (osg.`id_order_state_group` = a.`current_state`)
		LEFT JOIN `'._DB_PREFIX_.'order_state_lang` osl ON (os.`id_order_state` = osl.`id_order_state` AND osl.`id_lang` = '.(int)$this->context->language->id.')';
        $this->_orderBy = 'id_order';
        $this->_orderWay = 'DESC';
        $this->_where = '';
        $this->_use_found_rows = true;
        $statuses = OrderState::getOrderStates((int)$this->context->language->id, (int)$this->context->shop->id);
        foreach ($statuses as $status) {
           $this->statuses_array[$status['id_order_state']] = $status['name'];
        }
        $employees = Employee::getEmployeesByShop(true,$this->context->shop->id);
        foreach ($employees as $employee) {
//           $this->employees_array[$employee['id_employee']] = $employee['lastname'].' '.$employee['firstname'];
           $this->employees_array[$employee['id_employee']] = $employee['name'];
        }

        $trcs = Trc::getTrcs($this->context->shop->id);
        foreach ($trcs as $trc) {
...
/*         $groups = OrderStateGroup::getOrderStateGroups();
        foreach ($groups as $group) {
           $this->statuses_groups_array[$group['id_order_state_group']] = $group['group_name'];
        }
 */ 
        ...
        $this->fields_list = array_merge($this->fields_list, array(
            ...
            'blocker' => array(
                'title' => $this->l('Блок'), //PP Блок
                'align' => 'text-center',
                'callback' => 'printBlockerIcon',
                'type' => 'bool'
            ),
            ...
            'employee' => array(
                'title' => $this->l('Менеджер'),
                'type' => 'select',
                'list' => $this->employees_array,
            ...
            'trc' => array(
                'title' => $this->l('ТК'),
                'type' => 'select',
                'list' => $this->trcs_array,
                'filter_key' => 'trc!id_trc',
                'filter_type' => 'int',
                'order_key' => 'trc'
            )
        ));

        if ((Country::isCurrentlyUsed('country', true)) && false) {
        ...
        $this->bulk_actions = array(
			...
			'delete' => array('text' => $this->l('Удалить отмеченное'), 'confirm' => $this->l('Удалить выбранные позиции?'))
        );

		function my_getDiscountForOrder($my_id_order, $tot_prod)
		{
		/**
...
// 	 		$my_id_oder = $id_order;
			$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('
			SELECT DISTINCT ocr.`id_order_cart_rule`, ocr.`id_cart_rule`
				FROM `'._DB_PREFIX_.'order_cart_rule` ocr WHERE ocr.`id_order` = '.$my_id_order);

			$product_list = 'Products -* ';
			$my_discount_sum = 0;
			$my_discount_sum_tax_excl = 0;
			$my_perc = 0;
				foreach ($result as $row) {
 					$my_order_cart_rule = new OrderCartRule((int)$row['id_order_cart_rule']);
					$my_cart_rule = new CartRule($my_order_cart_rule->id_cart_rule);
					if ($my_cart_rule->reduction_percent <> 0) {
						$my_perc = $my_cart_rule->reduction_percent;
					} else {
						$my_perc = 0;
					}
					if ($my_perc <> 0) {
						$my_order_cart_rule->value = round($tot_prod * $my_perc / 100,2);
						$my_order_cart_rule->value_tax_excl = round(($my_order_cart_rule->value / 1),2);
					}
					$my_discount_sum = $my_discount_sum + $my_order_cart_rule->value;
					$my_discount_sum_tax_excl = $my_discount_sum_tax_excl + $my_order_cart_rule->value_tax_excl;
					if (!$my_order_cart_rule->update()) {
						$this->errors[] = Tools::displayError('An error occurred while updating discount information.');
					}
				
					}
 	 /* 		$this->total_discount = $my_discount_sum;
			$this->total_discount_tax_incl = $my_discount_sum;
			$this->total_discount_tax_excl = $my_discount_sum_tax_excl;
			
			$this->total_paid = $this->total_products - $my_discount_sum;
			$this->total_paid_tax_incl = $this->total_products - $my_discount_sum;
			$this->total_paid_tax_excl = $this->total_products - $my_discount_sum_tax_excl;
	 */
	        return $my_discount_sum;
		}
    }
    public function printBlockerIcon($value, $order)
    {
        return sprintf(
            '<a class="%s js-change-blocker-val" href="/%s"><i class="%s"></i></a>',
...
//file_put_contents('somefile.txt', PHP_EOL.$text66, FILE_APPEND);
    //die('<pre>'.print_r($this).'</pre>');
        ...
        if (Tools::isSubmit('changeBlockerVal') && $this->id_object) {
            if ($this->tabAccess['edit'] === '1') { 
               $this->action = 'change_blocker_val';
            } else {
    ...
    /**
     * Toggle the blocker flag
     */
    public function processChangeBlockerVal()
    {
       //$text66 = '  ajax  - '.'Orders -  2*' ;
        //file_put_contents('somefile.txt', PHP_EOL.$text66, FILE_APPEND);
    ...
    public function renderForm()
    {
        ...
        $employees = Employee::getEmployeesByShop(true,$this->context->shop->id);
 		$employees = array_merge(array(
			'id_employee' => 0
		),$employees);

		$trcs = Trc::getTrcs($this->context->shop->id);
		$trcs = array_merge(array(
			'id_trc' => 0
		),$trcs);
        ...
        $this->context->smarty->assign(array(
            ...
            'cart' => $cart,
            'is_show_fio' => $this->is_show_fio,
 			'employees_list' => $employees,
 			'trcs_list' => $trcs,
 			'employees_list2' => $this->employees_array,
            ...
            'order_states' => OrderState::getOrderStates((int)Context::getContext()->language->id, (int)$this->context->shop->id),
    ...
    public function initToolbar()
    {
        if ($this->display == 'view') {
            ...
            $this->toolbar_title[] = sprintf($this->l('Order %1$s from %2$s %3$s'), $order->order_number, ($this->is_show_fio ? $customer->firstname : $customer->my_name), ($this->is_show_fio ? $customer->lastname : ' '));
    ...
    public function setMedia()
    {
        ...
        $this->addJS(_PS_JS_DIR_.'admin/orders-actions.js');
/*         $this->addJS(_PS_JS_DIR_.'admin/orders_disc.js');
 */
    ...
    {
        // Попытка для сохранения скидки Китай
//        if (Tools::isSubmit('submitCartKit')) {
...
//        }
        ...
        elseif (Tools::isSubmit('submitEmployee') && isset($order)) {
            if ($this->tabAccess['edit'] === '1') {
                $id_new_employee = Tools::getValue('id_new_employee');

                    if ($order->id_employee != $id_new_employee && $id_new_employee != 0) {
						$order->id_employee = $id_new_employee;
						$order->update();
                        Tools::redirectAdmin(self::$currentIndex.'&id_order='.(int)$order->id.'&vieworder&token='.$this->token);
                    } else {
                        $this->errors[] = Tools::displayError('Не выбран менеджер!');
                    }
            } else {
                $this->errors[] = Tools::displayError('You do not have permission to edit this.');
            }
        ...
        elseif (Tools::isSubmit('submitNDS') && isset($order)) {
            if ($this->tabAccess['edit'] === '1') {
                $isnds = Tools::getValue('isnds');
                $sfnom = Tools::getValue('sfnom');
                $sfdate = Tools::getValue('sfdate');

                    if ($isnds != '' && (($sfnom == '') || ($sfdate == '') || ($sfdate == '0000-00-00'))) {
                        $this->errors[] = Tools::displayError('Не указан номер или дата счета-фактуры!');
                    } else {
						$order->isnds = $isnds;
						$order->sfnom = $sfnom;
						$order->sfdate = $sfdate;
        ...
        elseif (Tools::isSubmit('submitTrc') && isset($order)) {
            if ($this->tabAccess['edit'] === '1') {
                $id_new_trc = Tools::getValue('id_new_trc');

                    if ($order->id_trc != $id_new_trc && $id_new_trc != 0) {
						$order->id_trc = $id_new_trc;
						$order->update();
                        Tools::redirectAdmin(self::$currentIndex.'&id_order='.(int)$order->id.'&vieworder&token='.$this->token);
                    } else {
                        $this->errors[] = Tools::displayError('Не выбрана ТК!');
                    }
            } else {
                $this->errors[] = Tools::displayError('You do not have permission to edit this.');
            }
        ...
        elseif (Tools::isSubmit('cancelProduct') && isset($order)) {
            ...
            if ($this->tabAccess['edit'] === '1') {
				...
				$datepaym = Tools::getValue('payment_date');

				$spospaym = Tools::getValue('payment_method');
				...
				$kaspaym = Tools::getValue('payment_transaction_id');
				$my_payment_kasses = array();
				foreach (my_payments::getMy_Payments_kass() as $mpt) {
				   $my_payment_kasses[$mpt['id_my_payments_kas']] = $mpt['name'];
				}
                ...
                } elseif (!Validate::isGenericName($spospaym)) {
                    $this->errors[] = Tools::displayError('The selected payment method is invalid.');
                } elseif (!Validate::isString($kaspaym)) {
                ...
                } elseif (!Validate::isDate($datepaym)) {
                    $this->errors[] = Tools::displayError('The date is invalid');
                } else {
					$new_my_payment = new my_payments();
					$new_my_payment->id_shop = (int)$this->context->shop->id;
					$new_my_payment->date_upd = $datepaym;
					$new_my_payment->date_payment = $datepaym;
					$new_my_payment->date_add = $datepaym;
					$new_my_payment->rperiod = Configuration::get('PS_ORDER_RPERIOD', null, null, $new_my_payment->id_shop);
					$new_my_payment->id_my_payments_spos = array_search($spospaym, $my_payment_methods) + 1;
					$new_my_payment->summa_all = $amount;
					$new_my_payment->summa_yes = $amount;
					$new_my_payment->summa_no = 0;
					$new_my_payment->id_my_payments_tip = 1;
					$new_my_payment->id_my_payments_vid = 1;
					$new_my_payment->id_my_payments_kas = array_search($kaspaym, $my_payment_kasses);
					$new_my_payment->order = $order->order_number;
					$new_my_payment->id_customer = $order->id_customer;
					$new_my_payment->prim = '';
					$new_my_payment->add_from = 1;
                    $new_my_payment->add();
					
					if (!$order->addOrderPayment($amount, $spospaym, $kaspaym, $currency, $datepaym, $order_invoice, $new_my_payment->id)) {
            ...
            if (!Configuration::get('PS_INVOICE', null, null, $order->id_shop)) {
                ...
                $order->setInvoice(true,$order->order_number);
                Tools::redirectAdmin(self::$currentIndex.'&id_order='.$order->id.'&vieworder&conf=4&token='.$this->token);
            }
			...
			// Добавить новую скидку к заказу
    ...
    public function renderMypanel()
    {
		if ((bool)Configuration::get('PS_ORDER_MY_CONTROL_PANEL', null, null, (int)$this->context->shop->id)) {
			$helper = new HelperKpiMypanel();
			$helper->order_statuses_groups = OrderStateGroup::getOrderStateGroups();
			$helper->order_counts = OrderStateGroup::getOrderPerGroupsCount();
			return $helper->generate();
		}
    ...
    public function renderView()
    {
        ...
        $this->toolbar_title = sprintf($this->l('Order #%1$d (%2$s) - %3$s %4$s'), $order->id, $order->order_number, ($this->is_show_fio ? $customer->firstname : $customer->my_name), ($this->is_show_fio ? $customer->lastname : ' '));
        ...
        $payment_kasses = array();
        foreach (my_payments::getMy_Payments_kass() as $mpt) {
           $payment_kasses[$mpt['id_my_payments_kas']] = $mpt['name'];
        }
...
//        $employees_array = array();
        ...
        $this->tpl_view_vars = array(
            ...
            'customer' => $customer,
			'customer_employee' => Employee::getEmployeeNameById($customer->id_employee),
			'order_employee' => Employee::getEmployeeNameById($order->id_employee),
			'employees_list' => $employees,
			'trcs_list' => $trcs,
            'gender' => $gender,
            'is_show_fio' => $this->is_show_fio,
            ...
            'payment_kasses' => $payment_kasses,
            'invoice_management_active' => Configuration::get('PS_INVOICE', null, null, $order->id_shop),
            'PS_INVOICE_NUMBER_ORDER' => (int)Configuration::get('PS_INVOICE_NUMBER_ORDER', null, null, $order->id_shop),
...
// Иванов 
        ...
        StockAvailable::updateQuantity($product->id, isset($product_informations['product_attribute_id']) ? $product_informations['product_attribute_id'] : null, (-1)*$product_informations['product_quantity'], new Shop($cart->id_shop));
// Иванов ^
	...
	// Сохраняет скидку на Китай в карточке покупателя
	public function ajaxProcessSetCustomerDiscountKit() {
		...
		$my_customer->skid_1 = Tools::getValue('customer_skid_kit');
		if (!$my_customer->update()) {
			$this->errors[] = Tools::displayError('An error occurred while updating customer information.');
		}
	}

	// Сохраняет скидку на Польшу в карточке покупателя
	public function ajaxProcessSetCustomerDiscountPol() {
	...
	$my_customer = new Customer(Tools::getValue('id_customer'));
		...
		$my_customer->skid_3 = Tools::getValue('customer_skid_pol');
		if (!$my_customer->update()) {
			$this->errors[] = Tools::displayError('An error occurred while updating customer information.');
		}
	}

	// Сохраняет скидку на Россию в карточке покупателя
	public function ajaxProcessSetCustomerDiscountRos() {
	...
	$my_customer = new Customer(Tools::getValue('id_customer'));
		...
		$my_customer->skid_2 = Tools::getValue('customer_skid_ros');
		if (!$my_customer->update()) {
			$this->errors[] = Tools::displayError('An error occurred while updating customer information.');
		}
	}
	...
	public function ajaxProcessSetOrderDiscountAll() {
 		...
 		$my_order = new Order(Tools::getValue('id_order'));

		if (!Validate::isLoadedObject($my_order)) {
			die(Tools::jsonEncode(array('result' => false,'error' => $order->id_order)));
		}
		$my_customer = new Customer(Tools::getValue('id_customer'));
		if (!Validate::isLoadedObject($my_customer)) {
			die(Tools::jsonEncode(array('result' => false)));
		...
		$my_order_sum_tax_excl = 0;
		// Для итого без скидки
		$old_order_sum_tax_incl = 0;
		$old_order_sum_tax_excl = 0;
//		$my_order_detail = array();
            foreach ($result as $row) {
				$my_order_detail = new OrderDetail((int)$row['id_order_detail']);

				$my_product = new Product($my_order_detail->product_id);
				// цена из карточки товара
				$my_product_price = $my_product->price;
				// Макс скидка из карточки товара
				...
				switch($my_product_country) {
					case 1: //  Скидка из карточки покупателя если Китай
							...
							//  Скидка из введенная менеджером если Китай
							$my_product_new_discount = $my_order->skid_1; 
							break;
				...
				$my_product_show_discount = min($my_product_price_discount, $my_product_max_discount);
				$my_order_detail->my_price_discount = $my_product_show_discount;
				
				// Ставка НДС
				$my_product_tax_rate = $my_order_detail->tax_rate/100+1;
				// Цена с НДС
				$my_new_product_price_tax_incl = round($my_product_price * (100 - $my_product_rab_discount) / 100,2);
				$my_order_detail->unit_price_tax_incl = $my_new_product_price_tax_incl;
				// Цена без НДС
				$my_new_product_price_tax_excl = round($my_new_product_price_tax_incl / $my_product_tax_rate,2);
				$my_order_detail->unit_price_tax_excl = $my_new_product_price_tax_excl;
				// Количество товара
				$my_product_quantity = $my_order_detail->product_quantity;
				// Сумма с НДС без скидки
				$my_old_total_price_tax_incl = round($my_product_price * $my_product_quantity,2);
				// Сумма с НДС со скидкой
				$my_new_total_price_tax_incl = round($my_new_product_price_tax_incl * $my_product_quantity,2);
				$my_order_detail->total_price_tax_incl = $my_new_total_price_tax_incl;
				// Сумма без НДС без скидки
				$my_old_total_price_tax_excl = round($my_old_total_price_tax_incl / $my_product_tax_rate,2);
				// Сумма без НДС со скидкой
				$my_new_total_price_tax_excl = round($my_new_total_price_tax_incl / $my_product_tax_rate,2);
				$my_order_detail->total_price_tax_excl = $my_new_total_price_tax_excl;
				// Сумма всего по заказу с НДС без скидки
				$old_order_sum_tax_incl = $old_order_sum_tax_incl + $my_old_total_price_tax_incl;
				// Сумма всего по заказу без НДС
				$old_order_sum_tax_excl = $old_order_sum_tax_excl + $my_old_total_price_tax_excl;
				// Сумма всего по заказу с НДС со скидкой
				$my_order_sum_tax_incl = $my_order_sum_tax_incl + $my_new_total_price_tax_incl;
				// Сумма всего по заказу без НДС
				$my_order_sum_tax_excl = $my_order_sum_tax_excl + $my_new_total_price_tax_excl;
				
				if (!$my_order_detail->update()) {
					$this->errors[] = Tools::displayError('An error occurred while updating order detail information.');
				}
				$product_list = $product_list.'-'.$my_product->max_discount;
			// End of foreach	
            }
		// общие скидки в заказе
		$my_old_total_discount = $my_order->total_discounts;	
		$my_old_total_discount_t_i = $my_order->total_discounts_tax_incl;	
		$my_old_total_discount_t_e = $my_order->total_discounts_tax_excl;	
		
		$my_order->total_products = $my_order_sum_tax_incl;
		$my_order->total_products_wt = $old_order_sum_tax_incl;
		$my_order->total_paid = $my_order_sum_tax_incl - $my_old_total_discount;
		$my_order->total_paid_tax_incl = $my_order_sum_tax_incl - $my_old_total_discount_t_i;
		$my_order->total_paid_tax_excl = $my_order_sum_tax_excl - $my_old_total_discount_t_e;
		// ***+discount
		if (!$my_order->update()) {
			$this->errors[] = Tools::displayError('An error occurred while updating order information.');
				}
		// То же самое в счет, если он есть 		
		if ($my_order->invoice_number > 0) {
            $my_order_invoice = new OrderInvoice($my_order->invoice_number);
			$my_order_invoice->total_products = $my_order_sum_tax_incl;
			$my_order_invoice->total_products_wt = $old_order_sum_tax_incl;
			$my_order_invoice->total_paid_tax_incl = $my_order_sum_tax_incl - $my_old_total_discount_t_i;
			$my_order_invoice->total_paid_tax_excl = $my_order_sum_tax_excl - $my_old_total_discount_t_e;
			if (!$my_order_invoice->update()) {
				$this->errors[] = Tools::displayError('An error occurred while updating invoice information.');
					}
        }
	...
	public function ajaxProcesssubmitMyRemovePayment() {
	
		$id_del_payments = Tools::getValue('my_payment_id');
		$amount = str_replace(',', '.', Tools::getValue('my_payment_sum'));

		$my_o_d_id = Tools::getValue('id_order');
		$my_o_d = new Order($my_o_d_id);
		$my_o_d->total_paid_real -= $amount;
		$my_o_d->update();
		
		$my_p_d = new OrderPayment($id_del_payments);
		$my_m_p = new my_payments($my_p_d->id_my_payments);
		$my_m_p->summa_yes -= $amount;
		$my_m_p->summa_no += $amount;
		if ($my_m_p->add_from == 1) {
			$my_m_p->delete();
		} else {
			$my_m_p->update();
		}	
//		$my_p_d->delete();
       ...
       Tools::redirectAdmin(self::$currentIndex.'&id_order='.$my_o_d_id.'&vieworder&conf=4&token='.$this->token);

	}
	
	// Сохраняет изменения оплат по заказу
	public function ajaxProcesssubmitMySavePayment() {
				$my_o_d = new Order(Tools::getValue('id_order'));
				$my_p_d = Tools::getValue('my_payment_id');
				$my_payment_date = Tools::getValue('my_payment_date');
				$my_payment_metod = Tools::getValue('my_payment_metod');
				$my_payment_transaction_id = Tools::getValue('my_payment_transaction_id');
                $amount = str_replace(',', '.', Tools::getValue('my_payment_sum'));
                $currency = new Currency(Tools::getValue('my_payment_currency'));
				$my_payment_invoice = Tools::getValue('my_payment_invoice');
				if (($my_payment_invoice == '') || ($my_payment_invoice == ' ')) {
					$order_has_invoice = false;
				} else {
					$order_has_invoice = true;
				}
                if ($order_has_invoice) {
                    $order_invoice = new OrderInvoice($my_payment_invoice);
                } else {
                    $order_invoice = null;
                }
//		die(Tools::jsonEncode(array('result' => false,'error' => $my_p_d.' - '.$my_payment_date.' - '.$my_payment_metod.' - '.$my_payment_transaction_id.' - '.$amount.' - '.$my_payment_invoice)));

				if (!Validate::isNegativePrice($amount) || !(float)$amount) {
die(Tools::jsonEncode(array('result' => false,'error' => ' Цена - ')));  
                  $this->errors[] = Tools::displayError('The amount is invalid.');
                } elseif (!Validate::isGenericName($my_payment_metod)) {
die(Tools::jsonEncode(array('result' => false,'error' => ' метод - ')));  
                    $this->errors[] = Tools::displayError('The selected payment method is invalid.');
                } elseif (!Validate::isString($my_payment_transaction_id)) {
die(Tools::jsonEncode(array('result' => false,'error' => ' Транз - ')));  
                    $this->errors[] = Tools::displayError('The transaction ID is invalid.');
                } elseif (!Validate::isLoadedObject($currency)) {
die(Tools::jsonEncode(array('result' => false,'error' => ' Валюа - ')));  
                    $this->errors[] = Tools::displayError('The selected currency is invalid.');
                } elseif ($order_has_invoice && !Validate::isLoadedObject($order_invoice)) {
die(Tools::jsonEncode(array('result' => false,'error' => ' Сче   т - ')));  
                    $this->errors[] = Tools::displayError('The invoice is invalid.');
                } elseif (!Validate::isDate($my_payment_date)) {
die(Tools::jsonEncode(array('result' => false,'error' => ' дата - ')));  
                    $this->errors[] = Tools::displayError('The date is invalid');
                } else {
					if (!$my_o_d->mySaveOrderPayment($my_p_d, $amount, $my_payment_metod, $my_payment_transaction_id, $currency, $my_payment_date, $order_invoice)) {
						die(Tools::jsonEncode(array('result' => false,'error' => $my_p_d.' no- '.$my_payment_date.' - '.$my_payment_metod.' - '.$my_payment_transaction_id.' - '.$amount.' - '.$my_payment_invoice)));
                        $this->errors[] = Tools::displayError('An error occurred during payment.');
                    } else {
						die(Tools::jsonEncode(array('result' => false,'error' => $my_p_d.' yes- '.$my_payment_date.' - '.$my_payment_metod.' - '.$my_payment_transaction_id.' - '.$amount.' - '.$my_payment_invoice)));
                        Tools::redirectAdmin(self::$currentIndex.'&id_order='.$order->id.'&vieworder&conf=4&token='.$this->token);
                    }
                }
...
//  Первая версия - берет product_price_tax_incl (налог) из браузера
...
// берет скидку из браузера
        $order_detail->my_real_discount = Tools::getValue('product_my_discount');

        // Calculate differences of price (Before / After) Первая версия - Налог Браузер минус БД
...
//  Иванов сюда перенесено обновление стока 
		...
		if (($old_quantity - ($order_detail->product_quantity)) < 0 )
		{
			...
			if (($stock_av_value + $old_quantity - ($order_detail->product_quantity)) < 0)
			{
				die(Tools::jsonEncode(array(
                'result' => false,
                'error' => sprintf(Tools::displayError('Too much! The maximum quantity - %d.'), $stock_av_value + $old_quantity),
				'new_qty' => $stock_av_value + $old_quantity
            )));
			}
		}
...
/* 			$my_discount_sum2 = my_getDiscountForOrder((int)$order_detail->id_order, (float)$order->total_products);
		...
		$order->total_discounts_tax_incl = $my_discount_sum2;
		$order->total_discounts_tax_excl = $my_discount_sum2_tax_excl;
		
		$order->total_paid = $order->total_products - $my_discount_sum2;
		$order->total_paid_tax_incl = $order->total_products - $my_discount_sum2;
		$order->total_paid_tax_excl = $order->total_products - $my_discount_sum2_tax_excl;
        if (isset($order_invoice)) {
            $order_invoice->total_discount_tax_incl = $my_discount_sum2;
            $order_invoice->total_discount_tax_excl = $my_discount_sum2_tax_excl;
 ...
 */
//  Иванов здесь было обновление стока ^
    ...
    public function ajaxProcessDeleteProductAllProductsFromOrder()
    {
        ...
        foreach ($OrderDetailList as $row) {
            $order_detail = new OrderDetail($row['id_order_detail']);
