...
class AdminCartRulesControllerCore extends AdminController
{
    public function __construct()
    {
        ...
        parent::__construct();
        $this->_filter = ' AND a.my_id_shop = '.(int)Context::getContext()->shop->id;
    }

    public function ajaxProcessLoadCartRules()
    {
        $type = $token = $search = '';
        $limit = $count = $id_cart_rule = $my_id_shop = 0;
        if (Tools::getIsset('my_id_shop')) {
            $limit = Tools::getValue('my_id_shop');
        }
...
//		 $this->tpl_form_vars['my_id_shop'] = $my_id_shop;
//		 $this->tpl_form_vars['my_id_shop'] = 4;
