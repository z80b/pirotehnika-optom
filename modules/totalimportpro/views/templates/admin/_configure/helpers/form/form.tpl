	...
	{elseif $field.step == 'step3'}
		{block name="field"}
			{if $input.type == 'feed_sample'}
				...
				<button type="button" id="nextRow" value="1" class="button btn btn-info">View Next Row</button>
			{elseif $input.type == 'operations'}
				</br>
				<!--Operations-->
				<table class="table" cellspacing="0" cellpadding="0" id="addOperation">
					<thead>
						<tr class="nodrag nodrop">
								...
								{if isset($previous_data.name) AND isset($previous_data.inputs[0])}
									<tr id="adjustment_row_{$row|escape:'htmlall':'UTF-8'}">
										<td class="left">
											...
											<input type="hidden" name="adjust[{$row|escape:'htmlall':'UTF-8'}][]" value="{$previous_data.function|escape:'htmlall':'UTF-8'}">
										</td>
										<td class="left">
											...
											{if isset($field_val.option) AND $field_val.option == 'addMore'}
												<a onclick="return addSub(this);" class="button btn btn-default"><span>More&nbsp;&rarr;&nbsp;</span></a>
											{/if}
											</td>
										</td>
					...
					<tfoot>
						<tr>
							...
							<td class="center">
								<a class="button btn btn-info" id="addOperation" onclick="addOperation();">{l s='Add Operation' mod='totalimportpro'}</a>
							</td>
						</tr>
					</tfoot>
				</table>
									...
									{if isset($fields_value['simple_names']['specific_price'][$i]) || $i == 0}
										<tr class="vert">
											<td class="mapping_field col-left">{l s='Specific Price' mod='totalimportpro'}
												{if $i == $field_count-1}
													<a style="float:right;" onclick="return addVert(this, false);" class="button btn btn-default">More&nbsp;&darr;&nbsp;</a>
												{/if}
											</td>
												...
												{for $i=0 to $field_count-1}
													...
													<tr class="hori vert">
														<td class="mapping_field col-left">
															{$pretty_name[0]|escape:'htmlall':'UTF-8'}
															{if $i+1 == $field_count}
																<a style="float:right;" onclick="return addVert(this, true);" class="button btn btn-default">More&nbsp;&darr;&nbsp;</a>
															{/if}
														</td>
