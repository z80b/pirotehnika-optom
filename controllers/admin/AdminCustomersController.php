...
class AdminCustomersControllerCore extends AdminController
{
    ...
    public $is_show_fio;
    public $is_show_own_folder;
    public $is_show_discount = false;
    public $is_show_show_skid = false;
    ...
    public function __construct()
    {
		...
		$this->is_show_own_folder = Configuration::get('PS_STORE_OWN_FOLDER', null, null, (int)$this->context->shop->id);
 		$this->is_show_discount = Configuration::get('PS_OWN_COUNTRY_DISCOUNT', null, null, (int)$this->context->shop->id);
 		$this->is_show_show_skid = Configuration::get('PS_IS_SHOW_DISCOUNT', null, null, (int)$this->context->shop->id);
        $titles_array = array();
...
//           $this->employees_array[$employee['id_employee']] = $employee['lastname'].' '.$employee['firstname'];
        ...
        $carriers = Carrier::getMyCarriers();
 ...
 */			
        ...
        $this->_select = '
        a.date_add, a.id_employee, a.lastname, a.firstname, gl.name as title,  (
			SELECT CONCAT(e.`lastname`, \'. \', LEFT(e.`firstname`, 1)) 
            FROM '._DB_PREFIX_.'employee e
            WHERE e.id_employee = a.id_employee
		) AS `employee`, (
 ...
 //       $this->_join = 'LEFT JOIN '._DB_PREFIX_.'employee e ON (a.id_employee = e.employee)';
        ...
        switch ($this->display) {
            ...
            case 'view':
                ...
                if (($customer = $this->loadObject(true)) && Validate::isLoadedObject($customer)) {
                    array_pop($this->toolbar_title);
                    $this->toolbar_title[] = sprintf($this->l('Information about Customer: %s'), ($this->is_show_fio ? Tools::substr($customer->firstname, 0, 1).'. '.$customer->lastname : $customer->my_name));
                }
                break;
            case 'add':
            case 'edit':
                ...
                /** @var Customer $customer */
                if (($customer = $this->loadObject(true)) && Validate::isLoadedObject($customer)) {
                    $this->toolbar_title[] = sprintf($this->l('Editing Customer: %s'), ($this->is_show_fio ? Tools::substr($customer->firstname, 0, 1).'. '.$customer->lastname : $customer->my_name));
                } else {
    ...
    {
        parent::initProcess();
//$text66 = '  ajax  - '.'Customers -  1*'.$this->id_object ;
...
echo '<script>console.log('.$obj->id.')</script>';
		...
		$carriers = Carrier::getMycarriers($this->context->shop->id);
		$carriers = array_merge(array(
			'id_carrier' => 0
		),$carriers);
		...
		if ($this->is_show_discount == false) {
			$my_discount_arrayk = '';
			$my_discount_arrayr = '';
			$my_discount_arrayp = '';
			$my_discount_arrayg = '';
		...
		if ($this->is_show_show_skid == false) {
			...
			$my_show_skid_array = array(
                    'type' => 'switch',
                    'label' => $this->l('Разрешить показ цен со скидками'),
                    'name' => 'show_skid',
                    'required' => false,
                    'class' => 't',
                    'is_bool' => true,
			...
			$my_show_skid_array2 = array(
                    'type' => 'switch',
                    'label' => $this->l('Показывать цены без скидок'),
                    'name' => 'show_price_wo_skid',
                    'required' => false,
                    'class' => 't',
                    'is_bool' => true,
                    'values' => array(
                    ...
                    'hint' => $this->l('Отображать цену без скидки (перечеркнутая) или нет.')
                );
		}
		if ($this->is_show_own_folder == false) {
			$own_folder_array = '';
		} else {
			$own_folder_array = array(
						'type' => 'text',
						'label' => 'Персональная папка',
						'name' => 'own_folder',
						'required' => false,
						'col' => '4',
						'hint' => ''
...
//echo '<script>function skkk() {console.log(document.getElementById("skid_1").value);}</script>';
    ...
    public function renderView()
    {
        ...
        $orders = Order::getCustomerOrders($customer->id, true, null, 'admin');
        ...
        $this->tpl_view_vars = array(
            'customer' => $customer,
			'employee' => Employee::getEmployeeNameById($customer->id_employee),
            'is_show_fio' => $this->is_show_fio,
            'is_show_discount' => $this->is_show_discount,
            'is_show_show_skid' => $this->is_show_show_skid,
            'gender' => $gender,
    ...
    {
//$text66 = '  ajax  - '.'Customers -  2*'.$this->id_object ;
//file_put_contents('somefile.txt', PHP_EOL.$text66, FILE_APPEND);
    ...
    public function displayDeleteLink($token = null, $id, $name = null)
    {
        ...
        $name = ($this->is_show_fio ? $customer->lastname.' '.$customer->firstname : $customer->my_name);
...
return;
 */      
		$search_employee = (int)Tools::getValue('customer_employee');
		if ($search_employee > 0) {
			$customers = array();
			if ($results = Customer::searchByEmployeeId($search_employee)) {
     ...
     */
	 ...
	 public function ajaxProcessSearchCustomersById()
    {
/*             $to_return = array(
                'customers' => array(
					'id_customer' => '3',
 ...
 */      
        ...
        $searches = array_unique($searches);
        foreach ($searches as $search) {
            if (!empty($search) && $results = Customer::searchById($search, 50)) {
                foreach ($results as $result) {
                    if ($result['active']) {
                        $customers[$result['id_customer']] = $result;
                    }
