...
{block name="label"}
	...
	{/if}
	{if $input.type == "select_next_state"}
		<div id="tpl" style="display:{if isset($fields_value.array_next_state) && $fields_value.array_next_state}block{else}none{/if}">
	{/if}
	{if $input.type == "select_group_state"}
		<div id="tpl" style="display:{if isset($fields_value.array_group_state) && $fields_value.array_group_state}block{else}none{/if}">
	{/if}
	{$smarty.block.parent}
{/block}

{block name="input"}
	{if ($input.type == "select_template") || ($input.type == "select_next_state") || ($input.type == "select_group_state")}
		{if $input.type == "select_template"}
			<div class="col-lg-9">
				<div class="row">
					...
					<table class="table" id="table_check">
						<tbody>
							{foreach $fields_value.array_next_state as $next_state}
								<tr>
									<td><input type="checkbox" id="my_check_{$ir++}" name="is_check[]" value="{$next_state.id_order_state}" {if $next_state.checked == 1}checked="checked"{/if} onChange="refresh_array({$ir})"></td>
		...
		{if $input.type == "select_group_state"}
			<div class="col-lg-9">
				<div class="row">
					<table class="table" id="table_check">
						<tbody>
							{foreach $fields_value.array_group_state as $next_state}
								<tr>
									<td><input type="checkbox" id="my_check_{$ir2++}" name="is_check2[]" value="{$next_state.id_order_state_group}" {if $next_state.checked == 1}checked="checked"{/if} ></td>
									<td>{$next_state.name}</td>
								</tr>
							{/foreach}
						</tbody>
