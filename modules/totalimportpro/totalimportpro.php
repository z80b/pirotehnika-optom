...
<!-- End of HostJars Support Zendesk Widget script -->
	...
	public function import(&$settings, &$product_num = 0, &$limit = 0)
	{
		...
		while (($raw_prod = $this->getNextProduct($product_num)))
		{
			...
			{
				if (isset($raw_prod[$settings['field_names']['combination_'.$settings['combination_field']]]))
				{
	...
	*/
	public function importProduct(&$raw_prod, &$settings, &$update_id = 0, &$combination_id = 0)
	{
		...
		foreach ($this->languages as $lang)
		{
			if (!empty($raw_prod[$settings['field_names']['name'][$lang['id_lang']]]))
			{
				if (!empty($raw_prod[$settings['field_names']['link_rewrite'][$lang['id_lang']]]))
				{
					...
					{
						//make valid
						$raw_prod['link_rewrite'.$lang['id_lang']] = Tools::link_rewrite($raw_prod[$settings['field_names']['link_rewrite'][$lang['id_lang']]]);
						$settings['field_names']['link_rewrite'][$lang['id_lang']] = 'link_rewrite'.$lang['id_lang'];
					}
				}
				else
				{
					...
					$raw_prod['link_rewrite'.$lang['id_lang']] = Tools::link_rewrite($raw_prod[$settings['field_names']['name'][$lang['id_lang']]]);
					$settings['field_names']['link_rewrite'][$lang['id_lang']] = 'link_rewrite'.$lang['id_lang'];
				}
			}
		...
		foreach ($this->languages as $lang)
		{
			if (!empty($settings['field_names']['category'][$lang['id_lang']][0][0]))
			{
		...
		//category, if none set to default for new product
		if (!$category_mapped && !$update_id)
			$product->category = array((int)Configuration::get('PS_HOME_CATEGORY'));
		...
		if ($valid)
		{
			if ($update_id)
			{
				...
				if ($action)
				{
					...
					$log_text = 'Updated Product, Name: '.current($product->name);
			...
			else
			{
				...
				if ($action)
				{
					...
					$log_text = 'Added Product, Name: '.current($product->name);
					if (isset($product->reference))
						$log_text .= ', Reference: '.$product->reference;
					$this->logger->logInfo($log_text);
				}
		...
		if ($action)
		{
			...
			if (!empty($settings['field_names']['def_supplier']) || !empty($settings['field_names']['supplier']))
			{
			...
			//Combinations
			if (($combination_id || !empty($settings['field_names']['attribute']))
				&& !$product->is_virtual)
			{
	...
	public function importXML($filename, $product_tag, $xml_options)
	{
		...
		while ($data = fread($fh, 4096))
		{
			...
			if (!empty($xml_options['cron_feed']) && $this->total_items_ready == 1)
			{
				xml_parser_free($xml_parser);
				return true;
			}
		...
		$operations = array(
			...
			'splitCombinations' => array(
				...
				'inputs' => array(
					...
					array('type'=>'text', 'prepend'=>$this->l('Field to split combination fields on')),
	...
	private function _postProcess()
	{
		...
		if (Tools::isSubmit('submitStep'))
		{
			...
			elseif (Tools::getValue('submitStep5')) //only active with HJ_DEV or for disabling products in store but not file
			{
				...
				Tools::redirectAdmin($this->context->link->getAdminLink('AdminProducts').'&conf=50');
			}
		}
	...
	protected function displayForm()
	{
		...
		$helpdesk = array(
			'step1' => 'http://helpdesk.hostjars.com/entries/45206555-Step-1-Fetch-Feed',
			'step2' => 'http://helpdesk.hostjars.com/entries/44949029-Step-2-Global-Settings',
			'step3' => 'http://helpdesk.hostjars.com/entries/45216155-Step-3-Operations',
			'step4' => 'http://helpdesk.hostjars.com/entries/45216665-Step-4-Field-Mapping',
			'step5' => 'http://helpdesk.hostjars.com/entries/45217545-Step-5-Import',
		);
		...
		if (!empty($saved_settings))
		{
			...
			$this->_html .= '
					...
					<a href="#" class="button btn btn-info" id="loadProfile"><span>Load</span></a>
					<a href="#" class="button btn btn-info" id="deleteProfile"><span>Delete</span></a>
				</div>
				</div>
			</div>';
		}
		...
		foreach ($page_buttons as $page => $page_info)
		{
			...
			$this->_html .= '<a href="'.$page_info['link'].'" class="button col-sm-8 btn '.$page_btn_class.'"><span>'.$page_info['button'].'</span></a>';
			$this->_html .= '</label>';
		...
		<div class="panel">
			...
			<div class="list-group">
				<a class="list-group-item _blank" href="'._MODULE_DIR_.$this->name.'/sample_feeds/complete_1_ps.csv" target="_blank">
					Sample Products file (CSV)
				</a>
				<a class="list-group-item _blank" href="'._MODULE_DIR_.$this->name.'/sample_feeds/complete_1_ps.xml" target="_blank">
				...
				array(
					...
					'fields' => array(
						...
						array(
							...
							'name' => 'cron_feed',
							'id' => 'cron_feed',
							'desc' => $this->l('Recommended for large feeds. Only loads the first product to configure the settings with'),
						),
					),
				),
			),
		...
		function addOperation()
		{
			...
			if (operations[selected_op])
			{
				...
				for (i in inputs)
				{
					...
					{
						html += \'<select name="adjust[\' + operation_row + \'][]"><option>'.$this->l('-- Select --').'</option>\';';
			...
			$.ajax({
				...
				success: function(json) {
					if (json.length != 0 && json != 0) {
						...
						$.each($(\'#sampleFields thead tr th\'), function(i, item) {
	...
	protected function displayStep4()
	{
			...
			<script type="text/javascript">
				...
				function addVert(el, multi)
				{
					...
					if (multi == true)
					{
						...
						count = count + 1;
						newEl = newEl.split(\'][\'+(count-1).toString()+\'][]\').join(\'][\'+count.toString()+\'][]\');
					}
					$(el).hide();
				...
				function addCombo(el, multi)
				{
					newEl = \'<tr class="combo">\' + $(el).closest(\'.combo\').html() + \'</tr>\';
					$(el).hide();
					$(el).closest(\'.combo\').after(newEl);
					return false;
				}
				...
				function hideOtherLanguages(caller, lang_code, lang_name) {
					...
					caller.closest(\'.lang-select-group\').children(\'.lang-select\').hide();
					caller.closest(\'.lang-select-group\').children(\'.lang-\' + lang_code).show();
				}
			</script>
		';
		...
		$field_map = array(
			...
			'combination_ean13' => $this->l('Combination EAN13/Jan'),
	...
	protected function displayStep5()
	{
		...
		//redirect to product page
		$redirect = $this->context->link->getAdminLink('AdminProducts').'&conf=50'; //add in /*.'&conf=50'*/
		$this->_html .= '
		<style>
			...
			case 'step5':
				...
				if (version_compare(_PS_VERSION_, '1.6', '<'))
				{
					...
					$this->toolbar_btn['save_settings'] = array(
						...
						'href' => '',
						'js' => 'avascript:settingsProfile();return false;', //onclick
				...
				foreach ($item as $prod_id => $combination_image)
				{
					...
					foreach ($combination_image as $combo_id => $combo_image)
					{
						...
						if ($image_id)
						{
							...
							$this->associateFieldToShop($image->id, 'image');
	...
	private function setCategories($category_names, &$raw_prod)
	{
		...
		foreach ($languages as $lang)
		{
			$mapped_cats = array_keys($category_names[$lang]);
			foreach ($mapped_cats as $cat_num)
			{
				$cats = array();
				foreach ($category_names[$lang][$cat_num] as $key => $cat_column)
				{
					if (!empty($raw_prod[$cat_column]))
					{
						$cat = trim($raw_prod[$cat_column]);
						$cat = str_replace('&amp;', '&', $cat);
						$cat = str_replace('&amp;amp;', '&', $cat);
						$cats[$key] = $cat;
					}
		...
		$category_ids = array();
		// each $path will be a new cat tree
		foreach ($total_fields as $i => $cat_path)
		{
			if (!in_array(Configuration::get('PS_HOME_CATEGORY'), $category_ids))
				$category_ids[] = Configuration::get('PS_HOME_CATEGORY'); //first cat id
			...
			$fields_added = array();
			foreach ($cat_path as $lang => $subs)
			{
				$level = $parent_level + 1; //depth for sub_cat
							...
							if (!$cat_id)
							{
								...
								if ($field_error === true && $lang_field_error === true)
								{
									if ($category->add())
									{
										...
										$parent = $category->id; //new parent for next cat
										$fields_added[$sub_cat_num] = $parent; //keep record that this cat was assigned
										if (!in_array($cat_id, $category_ids))
											$category_ids[] = $parent; //add to return cats
									}
								}
							}
							else
							{
								// Associate with existing category
								$parent = $cat_id;
	...
	private function setCombinations(&$raw_prod, &$settings, &$product_id, &$combination_id, &$attribute_ids)
	{
		...
		if ($combination_id)
		{
			...
			if (isset($raw_prod[$settings['field_names']['combination_reference']]))
			{
				if (!Validate::isReference($raw_prod[$settings['field_names']['combination_reference']]))
					$raw_prod[$settings['field_names']['combination_reference']] = preg_replace('/^[^<>;={}]*$/u', '', $raw_prod[$settings['field_names']['combination_reference']]);
				$combination->reference = $raw_prod[$settings['field_names']['combination_reference']];
			}
			if (isset($raw_prod[$settings['field_names']['combination_ean13']]))
				$combination->ean13 = $raw_prod[$settings['field_names']['combination_ean13']];

			if (isset($raw_prod[$settings['field_names']['combination_upc']]))
				$combination->upc = $raw_prod[$settings['field_names']['combination_upc']];

			if (isset($raw_prod[$settings['field_names']['combination_wholesale_price']]))
			{
				$raw_prod[$settings['field_names']['combination_wholesale_price']] = preg_replace('/^[^\d]+/', '', $raw_prod[$settings['field_names']['combination_wholesale_price']]);
				$raw_prod[$settings['field_names']['combination_wholesale_price']] = str_replace(',', '.', $raw_prod[$settings['field_names']['combination_wholesale_price']]);
				$combination->wholesale_price = $raw_prod[$settings['field_names']['combination_wholesale_price']];
			}

			if (isset($raw_prod[$settings['field_names']['combination_price']])) //impact on price
			{
				$raw_prod[$settings['field_names']['combination_price']] = preg_replace('/^[^\d-]+/', '', $raw_prod[$settings['field_names']['combination_price']]);
				$raw_prod[$settings['field_names']['combination_price']] = str_replace(',', '.', $raw_prod[$settings['field_names']['combination_price']]);
				$combination->price = $raw_prod[$settings['field_names']['combination_price']];
			}

			if (isset($raw_prod[$settings['field_names']['combination_unit_price_impact']]))
			{
				$raw_prod[$settings['field_names']['combination_unit_price_impact']] = preg_replace('/^[^\d-]+/', '', $raw_prod[$settings['field_names']['combination_unit_price_impact']]);
				$raw_prod[$settings['field_names']['combination_unit_price_impact']] = str_replace(',', '.', $raw_prod[$settings['field_names']['combination_unit_price_impact']]);
				$combination->unit_price_impact = $raw_prod[$settings['field_names']['combination_unit_price_impact']];
			}

			if (isset($raw_prod[$settings['field_names']['combination_quantity']]))
				$combination->quantity = $raw_prod[$settings['field_names']['combination_quantity']];

			if (isset($raw_prod[$settings['field_names']['combination_weight']]))
				$combination->weight = $raw_prod[$settings['field_names']['combination_weight']];

			if (isset($raw_prod[$settings['field_names']['combination_default_on']]))
				$combination->default_on = (bool)$raw_prod[$settings['field_names']['combination_default_on']];

			if (isset($raw_prod[$settings['field_names']['combination_minimal_quantity']]))
				$combination->minimal_quantity = $raw_prod[$settings['field_names']['combination_minimal_quantity']];

			//image
			//needs to be added after the import
			if (!empty($raw_prod[$settings['field_names']['combination_image']]))
				$this->associations['combination_image'][$product_id][$combination->id] = $raw_prod[$settings['field_names']['combination_image']];

			$field_error = $combination->validateFields($this->validate);
			$lang_field_error = $combination->validateFieldsLang($this->validate);
			...
			if ($field_error === true && $lang_field_error === true)
			{
				if ($combination->save());
				{
					...
					$specific_shops = $this->getSpecificPriceShops($product->id, $combination_id);
					if ($specific_shops)
						$this->deleteSpecificPriceByShop($product->id, $combination_id);

					if (isset($raw_prod[$settings['field_names']['combination_specific_price'][0]]))
						$this->setSpecificPrices($raw_prod, $settings, $product->id, $combination_id);

					if (isset($combination->quantity))
						StockAvailable::setQuantity($product->id, $combination_id, (int)$combination->quantity);

					$this->logger->logInfo(sprintf('Updated Combination: %s, for Product Name: %s', $combination->reference, current($product->name)));
		...
		else //create new combination only if the attribute group name is set
		{
			if ($attribute_ids)
			{
				$combination = array(
				...
				if (isset($raw_prod[$settings['field_names']['combination_ean13']]))
					$combination['ean13'] = $raw_prod[$settings['field_names']['combination_ean13']];
				elseif (isset($product->ean13))
					$combination['ean13'] = $product->ean13;

				if (isset($raw_prod[$settings['field_names']['combination_upc']]))
					$combination['upc'] = $raw_prod[$settings['field_names']['combination_upc']];
				elseif (isset($product->upc))
					$combination['upc'] = $product->upc;
				...
				if (isset($raw_prod[$settings['field_names']['combination_price']]))
				{
					$raw_prod[$settings['field_names']['combination_price']] = preg_replace('/^[^\d-]+/', '', $raw_prod[$settings['field_names']['combination_price']]);
					$raw_prod[$settings['field_names']['combination_price']] = str_replace(',', '.', $raw_prod[$settings['field_names']['combination_price']]);
					$combination['price'] = $raw_prod[$settings['field_names']['combination_price']];
				...
				if (isset($settings['field_names']['combination_quantity']))
				{
					if (isset($raw_prod[$settings['field_names']['combination_quantity']]))
						$combination['quantity'] = $raw_prod[$settings['field_names']['combination_quantity']];
				...
				$image_id = 0; //associate after all products added
				...
				if ($id_product_attribute)
				{
					...
					if (!isset($raw_prod[$settings['field_names']['combination_image']]))
						$this->associations['combination_image'][$id_product_attribute] = $raw_prod[$settings['field_names']['combination_image']];

					//Specific Prices
					if (isset($raw_prod[$settings['field_names']['combination_specific_price'][0]]))
						$this->setSpecificPrices($raw_prod, $settings, $product->id, $id_product_attribute);

					Product::updateDefaultAttribute($product_id);
					foreach ($attribute_ids as $attribute_value)
					{
						Db::getInstance()->execute('
								INSERT IGNORE INTO '._DB_PREFIX_.'product_attribute_combination (id_attribute, id_product_attribute)
								VALUES ('.(int)$attribute_value.','.(int)$id_product_attribute.')');
					}

					StockAvailable::setQuantity($product->id, $id_product_attribute, (int)$combination['quantity']);
					$this->logger->logInfo(sprintf('Added Combination: %s, for Product Name: %s', $combination['reference'], current($product->name)));
	...
	*/
	...
	private function endTag($parser, $name)
	{
		if (strcmp($name, $this->product_tag) == 0)
		{
			...
			if (!(!empty($this->xml_cron_fetch) && $this->total_items_ready == 1))
			{
	...
	*/
		...
		foreach ($languages as $lang)
		{
			...
			foreach ($subs as $key)
			{
					...
					&& !empty($raw_prod[$attribute_field]))
				{
					...
					if ($value)
						$total_attr[$key][$lang][$attribute] = $value;
				}
			}
		}
		return $total_attr;
	...
	public function splitCombinations($adjustment)
	{
		...
		{
			$product_id_field_val = $row[$product_id_field];
			$exploded_fields = array();

			// First update the original row
			$sql = 'UPDATE `'._DB_PREFIX_.'hj_import` SET ';

			$max_split_num = 0;
			foreach ($combination_fields as $combination_field)
			{
				$exploded_fields[$combination_field] = explode($split_delim, $row[$combination_field]);
			...
			$sql = rtrim($sql, ', ').' WHERE `hj_id` = '.$row['hj_id'];
			Db::getInstance()->execute($sql);
