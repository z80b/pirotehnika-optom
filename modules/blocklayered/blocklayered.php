...
class BlockLayered extends Module
{
	...
	public function getProductByFilters($selected_filters = array())
	{
		...
		$my_is_show=(int)Configuration::get('PS_IS_SHOW_PRODUCTS',null,null,(int)Context::getContext()->shop->id);
		
		$query_filters_where = ' AND '.$alias_where.'.`active` = 1 AND '.$alias_where.'.`visibility` IN ("both", "catalog")';
		$query_filters_where .= ($my_is_show ? '' : ' AND IFNULL(sa.quantity, 0) > 0 ' );
		$query_filters_from = '';
		$query_filters_from .= ($my_is_show ? '' : 'LEFT JOIN `'._DB_PREFIX_.'stock_available` sa ON (sa.id_product = p.id_product '.StockAvailable::addSqlShopRestriction(null, null,  'sa').') ');
		...
		else
		{
			...
			if (version_compare(_PS_VERSION_, '1.6.1', '>=') === true)
			{
				...
				($my_is_show ? '' : ' AND IFNULL(stock.quantity, 0) > 0 ' ).'
...
//		$category_title = (empty($category->meta_title) ? $category->name : $category->meta_title);
		...
		if (version_compare(_PS_VERSION_, '1.6.0', '>=') === true) {
			$vars = array_merge($vars, array('pagination_top' => $smarty->assign('paginationId', 'top')
