...
{else}
	...
	<p class="cart_navigation clearfix">
		{if !$opc}
			<a  href="{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')|escape:'html':'UTF-8'}{else}{$link->getPageLink('order', true, NULL, 'step=1')|escape:'html':'UTF-8'}{/if}" class="button btn btn-default standard-checkout button-medium" onclick="yaCounter46713966.reachGoal('STEP0'); return true;" title="{l s='Proceed to checkout'}">
				<span>{l s='Оформить заказ'}<!-- <i class="icon-chevron-right right"></i> --></span>
			</a>
		{/if}
		...
		<div id="cart_summary" class="ps-order__table table-bordered {if $PS_STOCK_MANAGEMENT}stock-management-on{else}stock-management-off{/if}">
			<!-- <thead>
				<tr>
					...
					<th class="cart_total item text-right">{l s='ИТОГО, К ОПЛАТЕ:'}</th>
				</tr>
			</thead> -->
		...
		<table>
			<tfoot>
				...
				{if $use_taxes}
					{if $priceDisplay}
						<tr class="cart_total_price">
							<td rowspan="{$rowspan_total}" colspan="3" id="cart_voucher" class="cart_voucher">
								{if $voucherAllowed}
									<form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
										<fieldset>
											<!-- <h4>{l s='Vouchers'}</h4> -->
											<input type="text" class="discount_name form-control" id="discount_name" placeholder="Введите купон" name="discount_name" value="{if isset($discount_name) && $discount_name}{$discount_name}{/if}" />
							...
							<td colspan="{$col_span_subtotal}" class="text-right">{if $display_tax_label}{l s='Стоимость:'}{else}{l s='Стоимость:'}{/if}</td>
					...
					{else}
						<tr class="cart_total_price">
								...
								{if $voucherAllowed}
									<form action="{if $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" id="voucher">
										<fieldset>
											...
											<input type="text" class="discount_name form-control" id="discount_name" placeholder="Введите купон" name="discount_name" value="{if isset($discount_name) && $discount_name}{$discount_name}{/if}" />
											<input type="hidden" name="submitDiscount" />
											<button type="submit" name="submitAddDiscount" class="button btn btn-default button-small"><!-- <span> -->{l s='Применить'}<!-- </span> --></button>
										</fieldset>
									</form>
									{if $displayVouchers}
										<p id="title" class="title-offers">{l s='Take advantage of our exclusive offers:'}</p>
							...
							<td colspan="2" class="text-right">{if $display_tax_label}{l s='Стоимость:'}{else}{l s='Стоимость:'}{/if}</td>
							<td colspan="2" class="price" id="total_product"><span>{displayPrice price=$total_products_wt}</span></td>
						</tr>
					{/if}
				{else}
					<tr class="cart_total_price">
				...
				{if $use_taxes && $show_taxes && $total_tax != 0 }
					{if $priceDisplay != 0}
					<tr class="cart_total_price">
						<td colspan="{$col_span_subtotal}" class="text-right">{if $display_tax_label}{l s='Total (tax excl.)'}{else}{l s='ИТОГО, К ОПЛАТЕ:'}{/if}</td>
	...
	<p class="cart_navigation clearfix">
		{if !$opc}
			<a  href="{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')|escape:'html':'UTF-8'}{else}{$link->getPageLink('order', true, NULL, 'step=1')|escape:'html':'UTF-8'}{/if}" class="button btn btn-default standard-checkout button-medium" onclick="yaCounter46713966.reachGoal('STEP0');" title="{l s='Proceed to checkout'}">
				<span>{l s='Оформить заказ'}<!-- <i class="icon-chevron-right right"></i> --></span>
			</a>
		{/if}
		<!-- <a href="{if (isset($smarty.server.HTTP_REFERER) && ($smarty.server.HTTP_REFERER == $link->getPageLink('order', true) || $smarty.server.HTTP_REFERER == $link->getPageLink('order-opc', true) || strstr($smarty.server.HTTP_REFERER, 'step='))) || !isset($smarty.server.HTTP_REFERER)}{$link->getPageLink('index')}{else}{$smarty.server.HTTP_REFERER|escape:'html':'UTF-8'|secureReferrer}{/if}" class="button-exclusive btn btn-default" title="{l s='Continue shopping'}">
