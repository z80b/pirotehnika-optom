...
// Иванов 
		...
		$this->stock_cover_warehouses = Warehouse::getWarehouses(false,$this->id_shop);
		foreach ($this->stock_cover_warehouses as $items) {
...
// Иванов ^		
    ...
    public function renderDetails()
    {
        if (Tools::isSubmit('id_product')) {
            ...
            if ($warehouse != -1) {
				...
				$where_warehouse = ' AND a.id_warehouse IN ('.$this->wh_list.')';
			}
						  ...
						  (
						  	SELECT SUM(s.usable_quantity) as usable_quantity, s.id_product_attribute, s.reference, s.ean13, s.upc
						   	FROM '._DB_PREFIX_.'stock s
								leftJoin warehouse_shop ws on ws.id_warehouse = s.id_warehouse
    ...
    public function renderList()
    {
        ...
        if ($this->getCurrentCoverageWarehouse() != -1) {
			...
			$this->_where .= ' AND s.id_warehouse IN ('.$this->wh_list.')';
		}

        // Hack for multi shop ..
        $this->_where .= ' AND b.id_shop = '.$this->id_shop;
        ...
        if ($this->display == 'details') {
            ...
            for ($i = 0; $i < $nb_items; ++$i) {
                ...
                $coverage = StockManagerFactory::getManager()->getProductCoverage(
                    ...
                    (($this->getCurrentCoverageWarehouse() == -1) ? $this->whs_id : Tools::getValue('id_warehouse', -1))
                );
                if ($coverage != -1) {
            ...
            for ($i = 0; $i < $nb_items; ++$i) {
                ...
                if (array_key_exists('variations', $item) && (int)$item['variations'] <= 0) {
                    ...
                    $coverage = StockManagerFactory::getManager()->getProductCoverage(
                                    ...
                                    (($this->getCurrentCoverageWarehouse() == -1) ? $this->whs_id : $this->getCurrentCoverageWarehouse())
                    );
                    if ($coverage != -1) {
    ...
    protected function getQuantitySold($id_product, $id_product_attribute, $coverage)
    {
        ...
        if ($this->getCurrentCoverageWarehouse() != -1) {
            $query->where('od.id_warehouse = '.(int)$this->getCurrentCoverageWarehouse());
//            $query->where('od.id_warehouse = 1');
        } else {
			$query->where('od.id_shop = '.(int)$this->id_shop);
//			$query->where('od.id_shop = 1');
		}
