...
class CategoryCore extends ObjectModel
{
    ...
    public function getProducts($id_lang, $p, $n, $order_by = null, $order_way = null, $get_total = false, $active = true, $random = false, $random_number_products = 1, $check_access = true, Context $context = null)
    {
      if (!$context) {
        ...
        $id_manuf = (int)Tools::getValue('id_manufacturer');
        ...
        if ($get_total) {
            $sql = 'SELECT COUNT(cp.`id_product`) AS total
					...
					LEFT JOIN `'._DB_PREFIX_.'category_product` cp ON p.`id_product` = cp.`id_product`'.Product::sqlStock('p', 0).'
					WHERE cp.`id_category` = '.(int)$this->id.
				($this->my_is_show ? '' : ' AND IFNULL(stock.quantity, 0) > 0 ' ).
                ($front ? ' AND product_shop.`visibility` IN ("both", "catalog")' : '').
                ($active ? ' AND product_shop.`active` = 1' : '').
                ($id_manuf ? ' AND p.id_manufacturer = '.(int)$id_manuf : '').
        ...
        $sql = 'SELECT p.*, p.price as price_discount, spr.reduction*100 as reduction, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) AS quantity'.(Combination::isFeatureActive() ? ', IFNULL(product_attribute_shop.id_product_attribute, 0) AS id_product_attribute,
					product_attribute_shop.minimal_quantity AS product_attribute_minimal_quantity' : '').', pl.`description`, pl.`description_short`, pl.`available_now`,
					pl.`available_later`, pl.`link_rewrite`, pl.`meta_description`, pl.`meta_keywords`, pl.`meta_title`, pl.`price_name` AS `name`, image_shop.`id_image` id_image,
				...
				LEFT JOIN `'._DB_PREFIX_.'specific_price` spr
					ON (spr.`id_product` = p.`id_product` AND spr.id_shop='.(int)$context->shop->id.')
				...
				($this->my_is_show ? '' : ' AND IFNULL(stock.quantity, 0) > 0 ' ).'
					...
					.($id_manuf ? ' AND p.id_manufacturer = '.(int)$id_manuf : '')
                    .($id_supplier ? ' AND p.id_supplier = '.(int)$id_supplier : '');
// Иванов ^ вставлена строка после WHERE product_shop.`id_shop` = '.(int)$context->shop->id.
// старая версия - нужно исправить stock.depends_on_stock == 0
//				($this->my_is_show ? '' : (	stock.depends_on_stock == 0 ? ' AND IFNULL(stock.quantity, 0) > 0 ' : ' AND (p.`quantity` > 0 OR IFNULL((SELECT SUM(`quantity`) FROM `'._DB_PREFIX_.'product_attribute` paz WHERE p.`id_product` = paz.`id_product`), 0) > 0) ')).'
        if ($random === true) {
		...
		if (Configuration::get('PS_IS_SHOW_DISCOUNT',null,null,(int)$context->shop->id) && $context->customer->show_skid) {
			foreach($result222 as &$e){
				if ($e['country_prois'] == 1) {
					...
					$disc_rate = $context->customer->skid_2;
				} elseif ($e['country_prois'] == 3) {
					$disc_rate = $context->customer->skid_3;
				} else {
					$disc_rate = 1;
				}
