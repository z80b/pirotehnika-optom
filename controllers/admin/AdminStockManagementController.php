...
// Иванов ^		
    ...
    public function renderList()
    {
        ...
        if (!empty($id_product)) {
            ...
            // overrides query
			// Иванов (SELECT SUM( .... 2 строки 
            $this->_select = 'IFNULL(pa.ean13, a.ean13) as ean13,
            IFNULL(pa.upc, a.upc) as upc,
            IFNULL(pa.reference, a.reference) as reference,
            (SELECT SUM(physical_quantity) FROM `'._DB_PREFIX_.'stock` WHERE id_product = a.id_product AND id_warehouse IN ('.$this->wh_list.')) as physical_quantity,
            (SELECT SUM(usable_quantity) FROM `'._DB_PREFIX_.'stock` WHERE id_product = a.id_product AND id_warehouse IN ('.$this->wh_list.')) as usable_quantity,
