...
<!--        <p class="title_block">{l s='List of pages in %s:' sprintf=$cms_category->name}</p>
 -->            
            <div class="ps-tabs">
                ...
                <a class="ps-tabs__button" href="{$link->getCMSLink($cmspages.id_cms, $cmspages.link_rewrite)|escape:'html':'UTF-8'}">
                    <span class="ps-tabs__text">{$cmspages.meta_title|escape:'html':'UTF-8'}</span>
                </a>
                {/foreach}
            </div>
        {/if}
